...
Shared-infrastructure acceptance is a pre-deployment gate, and none of it is satisfied today. It proves provider-neutral local mode plus the real Clever Cloud shared mode; TLS hostname/certificate rejection; the credential privilege level the provider actually grants, detected and reported rather than assumed; database-enforced application-role isolation and migration-role separation only where a provider permits creating database users, and their documented absence where it does not; server-time fenced host leases; idempotent concurrent writers; snapshot-committed/`catalog-pending` database outage recovery; PostgreSQL catalog rebuild from the repository snapshot list plus source rescans; coordinated PostgreSQL/repository/config-key backup documentation, including restic repository password custody; and an hourly user timer on each enabled source host after manual bootstrap.
...
- A PostgreSQL outage after a backup leaves the snapshot uncatalogued; any instance adopts it from the repository snapshot list, restoring visibility, ordering, and restic's counts without republishing bytes. The adopted row keeps `catalog-pending`: its record of which sessions the snapshot held was never written and is not derivable from the list, so that state is permanent and `archive status` reports it as such rather than as a pending action.
...
- add restore-and-rescan completion for `catalog-pending` snapshots, which is the only way to recover a snapshot's session detail once its owning host failed to write it: restore the snapshot to a private location, rescan it with the adapters that own its trees, publish the session rows it actually held, and mark it committed. Phase A deliberately ships without it, reporting the permanence instead of implying a fix;
