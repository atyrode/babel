...
- the encrypted remote archive, its credentials, retention, or backup schedule;
...
- **dotfiles owns transport and secrets:** Cellar credentials, crypt configuration, host prefixes, remote health, and restore/materialization;
- **Babel owns interpretation:** indexing a local snapshot and analyzing its contents.

The desired stable integration is a dotfiles command that materializes a read-only snapshot, conceptually:

```text
atyrode backup restore --sessions --destination PATH [--host HOST]
```

Until that command exists, the equivalent implementation may source the existing 0600 environment file and perform a read-only `rclone copy archive:… PATH`. Babel must not duplicate vault enrollment or persist another copy of the archive credentials.

A local directory is always a valid Babel input. This keeps the analyzer testable and usable independently of the operator-specific transport.

## 3. Source data and trust model

Supported sources initially are the session trees already archived by dotfiles:
...
Materialize selected host prefixes from the encrypted archive into a local snapshot. Fetch is read-only with respect to the remote and never uses `sync`. The snapshot records its source host, remote path, and fetch time.
...
```text
babel fetch [--host HOST] [--destination PATH]
babel ingest PATH...
...
- `babel run` is only orchestration for fetch/ingest/preflight/analyze/synthesize; it does not hide a distinct workflow.
...
- integrate read-only archive materialization through the dotfiles boundary;
...
2. The dotfiles system owns encrypted archive transport and credentials; Babel consumes local snapshots.
3. Local directories remain a first-class input.
4. Raw transcripts are untrusted and private; local inference is the default.
5. Analysis is recipe-based, versioned, evidence-constrained, and incremental.
6. The default analyzes new or invalidated material; full reanalysis is explicit.
7. Outputs distinguish observations, findings, and proposals.
8. Proposals may target repositories but are never published automatically.
9. Both harmful and effective interaction patterns are in scope.
10. The first implementation should prove one narrow end-to-end evidence loop before expanding adapters or recipes.
...
8. What stable restore/materialization interface should be added to `atyrode backup` so Babel never depends on rclone configuration internals?
