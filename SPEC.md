...
Babel's side of that handshake is defined and implemented ahead of Code's, so the gate is a conformance result rather than a negotiation. The protocol is newline-delimited JSON over the child's stdin and stdout, and the job document travels on stdin alone — never argv, never the environment — because it carries the run-scoped broker credential and both of those are visible in a process listing. A worker's first event in worker mode is its resolved configuration, which must name the profile the job named and must declare the containment it provides: its backend, whether the filesystem is confined, whether egress is denied by default, whether resources are bounded, whether the environment is disposable, and its own statement of what it does not contain. Babel refuses a declaration short of the run's requirement at that first event, which is before the worker executes any analysis but after the complete job document — source selectors, grant, and the run-scoped broker token — has already been written to the worker's stdin; splitting the job so the credentials follow the declaration is a §14 gate. Babel defaults to the strict requirement so an unset field fails closed, and records the declaration in the run receipt so a later reviewer sees which boundary produced the evidence rather than assuming the boundary current at review time. Because Babel cannot verify a sandbox from outside the process, these obligations are executable conformance tests that any implementation must pass, not descriptions it may claim to satisfy.
...
Each launch creates a 256-bit one-time bootstrap nonce. `babel web --open` places it only in the URL fragment of the loopback bootstrap page; fragments are never sent in HTTP requests. Embedded bootstrap code immediately removes the fragment with `history.replaceState`, posts the nonce in a request body under `Referrer-Policy: no-referrer`, and receives a rotated host-only `HttpOnly; SameSite=Strict` session cookie. The nonce is single-use and expires quickly; lock/stop revokes every nonce and session. No bearer credential, transcript content, or sensitive selector appears in a query string, request log, referrer, or retained history entry. That exchange is the requirement and not yet the built behavior: what ships is one per-launch token delivered in the fragment and then held in `sessionStorage` as a bearer credential for the process's lifetime, so the credential is reachable from JavaScript. The requirement stands and the shortfall is a §14 gate (decision 34).
...
The same command is how a second instance browses the shared catalog, so `archive status` reports what the catalog holds as well as what it lacks: per publishing host, its catalog snapshot rows, its distinct session identities, its `catalog-pending` rows, and the newest publication order with that row's snapshot time. It stays a separate view from the repository listing rather than merged columns, because the whole reason `publication_order` and the commit state exist is that the repository and the catalog can disagree. Cross-machine browsing of titles and other sensitive metadata still arrives with Phase B's encrypted rows: what Phase A exposes fleet-wide is opaque identity, ordering, counts, commit state, and timestamps.
...
The Phase A web shell is browser-driven against the actual server. Acceptance covers Home/Sessions/fetch/lock-stop behavior, narrow and wide layouts, keyboard navigation, malicious HTML/Markdown/URL/control fixtures, `Host`/`Origin`/CSRF/DNS-rebinding rejection, fragment-only delivery of the launch credential and its absence from every URL path, query string, and cookie, `no-store` responses, CSP/no-remote-assets/no-service-worker enforcement, and proof that no known transcript or credential sentinel reaches URLs, browser history, server logs, or cached responses. The nonce-to-cookie exchange §2.7 requires is not among them, because it is not built (§14).
...
34. The web server has no v1 remote/LAN/persistent mode and enforces `Host`/origin/CSRF checks, no-store, restrictive CSP, no external assets/service worker, universal untrusted rendering, and explicit lock/stop. Its session bootstrap is not one-time, and recording it as one would hide the exposure: one 256-bit token is generated at server start, delivered in the launch URL's fragment so it never reaches a request line, copied by the frontend into `sessionStorage`, and presented as an `Authorization: Bearer` header for the whole process lifetime — no nonce exchange, no cookie, no rotation. The consequence is stated rather than buried: because the credential lives in `sessionStorage` instead of an `HttpOnly` cookie, any future cross-site-scripting hole or frontend supply-chain compromise can read it out of the page, and the cookie design §2.7 requires is exactly what closes that channel. Today's controls bound the exposure without replacing the missing one: `default-src 'self'` with no remote assets and no service worker, no HTML-injection sink in the frontend, a token that never travels in a URL path, query string, or cookie, revocation on lock/stop, and a listener that dies with the process. The exchange is a §14 gate.
...
- Exchange the launch nonce for a session cookie the page cannot read: POST the bootstrap nonce once, receive a rotated host-only `HttpOnly; SameSite=Strict` session cookie, and expire the nonce on first use (§2.7, decision 34). **Not built, measured 2026-08-30.** What ships is one 256-bit token generated at server start, delivered in the URL fragment, copied into `sessionStorage`, and accepted as a bearer credential for the life of the process: nothing is single-use and nothing rotates. It matters because a credential the page can read is a credential any script in the page can exfiltrate, so a future cross-site-scripting hole or a compromised frontend dependency takes the live session, which is the precise channel an `HttpOnly` cookie closes. The present mitigations are real and are not that control: a restrictive `default-src 'self'` policy, no remote assets or service worker, no HTML-injection sink in the frontend, a token that never appears in a URL path, query string, or cookie, and lock/stop revoking it. Closing the gate also removes the frontend's import-order coupling, because the fragment would be read exactly once.

### Before Phase B exploration is accepted

- Select and threat-model the Linux and Darwin sandbox backends, including filesystem isolation, process/resource controls, teardown, and escape assumptions. **Ownership settled and enforced 2026-08-29** (decision 53): Code provides the disposable execution sandbox and credential isolation, Babel authorizes capabilities and owns process-tree lifetime. The worker protocol now makes the boundary declared rather than assumed — a worker's resolved-configuration event must name its backend, its filesystem, network, resource, and teardown properties, and its own escape assumption, and Babel refuses a declaration short of the run's requirement at that first event, before the worker executes any analysis. The strict requirement is the default so a forgotten field fails closed, relaxing it is per-run and visible, and even a relaxed run must still name a backend, because a receipt that names no boundary cannot tell a reviewer what the evidence was produced behind. What remains belongs to Code — the per-platform sandbox implementation and the written escape assumption it declares — and to Babel, whose refusal currently lands after the job document has already reached the worker (next gate).
- Stage the job document so the containment declaration precedes the run's credentials: a profile-and-limits preamble sufficient for the worker to resolve and declare its configuration, then the sources, the grant, and the broker token written only after that declaration satisfies the run's requirement. **Not built, measured 2026-08-30.** Today the complete job is written to the worker's stdin immediately after the handshake and the declaration is checked at the worker's first event, so a worker Babel refuses has already held the run-scoped broker token and the source selectors it was never authorized to receive. It is deferred rather than dismissed: it is a wire change in two separately developed repositories, and it moves the conformance obligations, since a worker would have to declare containment from the preamble alone and the suite would have to prove it does. The residual exposure is bounded because the token is scoped to the single run and the run is refused before any analysis executes; it is not eliminated, because nothing stops a worker from copying stdin before it declares anything.
