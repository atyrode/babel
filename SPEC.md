...
The pre-Babel dotfiles automation was a proven operational prototype, not Babel's compatibility contract. It archived OMP, Codex, and Claude Code trees hourly to a Clever Cloud Cellar bucket through `rclone crypt`, and was retired on 2026-08-29 (decision 52). It lacked the session-aware catalog, integrity verification, deduplicated growing-session storage, and selective restore Babel requires.
...
It ignores the legacy remote namespace entirely. Legacy objects remain untouched and recoverable with direct rclone, but Babel does not list, import, migrate, or preserve their layout. Source data still present on managed machines is captured by Babel's first restic push. Retirement was a clean dotfiles cutover, not an in-place migration, and it happened on 2026-08-29. The order this sentence originally required - keep the job enabled until a real Cellar round trip covers all three harnesses *and* the replacement timer is verified - was not followed: the bucket was deleted after the round trip but before the timer existed, and the still-enabled job re-created it and re-uploaded the corpus within the hour. Decision 52 records that and its two lasting consequences.
...
- retire the old backup job once all three adapters, shared reconciliation, and the replacement timer are proven. Done 2026-08-29: the job, its `atyrode backup` CLI surface, and its bucket are gone, and `rclone` left the profile with them (decision 52).

The Phase A Linux rollout contract is declarative and reversible. Dotfiles locks the Babel source revision and Nix derivation hash, renders `babel-archive.service` and `babel-archive.timer`, and records the realized executable path in bootstrap evidence. Activation configures and verifies storage first, creates the repository once per deployment with `babel archive init` — no other command creates one, so an activation that skips this step fails its first push with `no repository at <locator>` — performs the manual bootstrap/backfill and two-instance checks, then enables the timer. Rollback disables/stops the Babel timer, reactivates the immediately preceding successful dotfiles/Home Manager generation, and never deletes Babel snapshots or rewinds the shared catalog. Two constraints now bound it. There is no second automated copy to fall back on, so a disabled timer means no automated archive until it is re-enabled, while existing snapshots stay restorable with the restic binary alone and the sources stay on disk. And a rollback target from before the cutover generation would reinstall the retired job, whose `rclone copy` auto-creates its destination and would therefore re-create the deleted bucket and re-upload the whole corpus, so rollback targets must be at or after that generation. A replacement version must pass the same manual gate before its timer is re-enabled.
...
4. Babel uses a clean restic repository under its own prefix and ignores the legacy remote archive: the legacy data was never listed, imported, migrated, or preserved by Babel. Its original "never deleted" clause is superseded by decision 52 - the operator instructed deletion once Babel's copy was verified, and the bucket is gone.
...
52. The pre-Babel backup job is retired, and the order in which that happened is recorded because it was the wrong order (2026-08-29). §2.3 required keeping the job enabled until a real Cellar round trip covered all three harnesses *and* the replacement timer was verified. The round trip passed and the operator authorised deleting the legacy bucket, so it was deleted - but the replacement timer did not exist yet, and the still-enabled hourly job re-created the bucket and re-uploaded 8.8 GiB within the hour, because `rclone copy` auto-creates its destination. Nothing of *this* host's was lost: its sources were still on disk and Babel's copy had already been proven byte-identical by a restic-only restore. That scope matters, because the fleet declares five hosts and the cutover only takes effect on each as `atyrode apply` runs there. A second live host was still running the retired job at 19:08 UTC and re-created the bucket a second time, holding roughly 44 MiB under its own directory - so the bucket was never only this workstation's, and whatever that host had deleted locally while relying on it did not survive either deletion. The bucket is therefore left in place until every host has converged: the order this decision exists to record is retire the unit everywhere, *then* delete once. The cutover landed in `atyrode/dotfiles` - `babel-archive.service`/`.timer` replacing `atyrode-session-backup`, the `atyrode backup` CLI removed, `rclone` dropped from the profile, `babel` pinned as a flake input so the unit runs an absolute store path - and the bucket was deleted a second time, after the unit that recreated it was gone from *this* generation - which was still not enough, because a generation is per-host and another host had not applied it. Two consequences outlive the incident and are recorded in §12: a rollback target from before the cutover would reinstall the retired job and repeat the re-upload, and no second automated copy exists any more, so a broken Babel timer means no automated archive until it is fixed.
...
- Document and exercise coordinated PostgreSQL catalog backup, repository recovery, restic repository password custody and backup, storage configuration recovery, one-time repository creation, manual primary-host bootstrap, hourly timer enablement, and rollback now that no legacy backup remains behind it (decision 52). Repository creation, bootstrap, and timer enablement are exercised; the backup, custody, and rollback drills are not.
