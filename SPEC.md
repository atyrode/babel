...
Database URLs, encryption keys, and storage credentials remain in Babel's trusted control process and are never exposed to Code, OMP, sandboxes, recipes, browser state, tool arguments, query/bind logs, traces, or diagnostics. Every fully authorized instance can necessarily decrypt the shared corpus; compromise of one has that blast radius. Credential rotation and coordinated PostgreSQL/Cellar/key backups are real operational controls. Per-instance database credentials and their individual revocation are real controls only where the provider issues them: where one credential serves the whole deployment, no database-level control can revoke a single instance, and the honest remaining controls are fleet-wide rotation and repository-password custody. Migration authority is separable by privilege on the same condition; otherwise it rests on operator procedure. Application-level instance revocation closes the eviction gap without depending on database roles, and its authority model is deliberately recorded rather than implied: revocation is ordinary DML on `instances`, a table an instance must already write to register itself and refresh its last-seen time, so the call expresses an operator's decision without authenticating one. The least-privileged application role Babel issues therefore revokes any instance id and clears its own revocation, as does any credential holding `instances` DML; that follows from the grants the mechanism needs rather than from the provider, so it holds under per-instance credentials too. Revocation evicts a machine that is out of service and bounds a compromised one until it is noticed; it is not a control against a hostile credential holder, where fleet-wide rotation and repository-password custody are today's controls. Restricting revocation authority by privilege is a §14 decision and §12 Phase C work, not a property Babel has.
...
- implement the Phase A PostgreSQL schema, migrations, credential-privilege detection, application-level instance revocation that force-expires the revoked instance's leases and refuses its lease acquisition, renewal, and publication, server-time fenced host leases, idempotent shared catalog, snapshot-to-catalog reconciliation, and catalog rebuild;
...
- expand the approved source-host rollout and verify hourly archive, shared-catalog, reconciliation, and instance-revocation health across the managed fleet;
- add richer local retention/prune UX and operational health review;
- add per-instance database-role revocation wherever a provider permits creating database users, and with it column-level grants that reserve `instances.revoked_at` to an operator role so eviction becomes database-enforced rather than an operator convention, payload-key rotation, coordinated PostgreSQL/Cellar/key backup and restore drills, and monitoring;
...
- Prove local-path-repository/SQLite development mode, real Clever Cloud Cellar/PostgreSQL mode, detected credential privilege reported rather than assumed, application-level instance revocation, migration/application role separation wherever a provider permits creating database users, TLS failure behavior, concurrent writers, database/repository outages, two-instance cache rebuild/browse/fetch, and no PostgreSQL dependency in direct archive recovery: the restic binary plus the repository password alone restore every source tree.
- Decide whether instance revocation must be restricted by privilege, and record the answer either way. Revocation is `instances` DML, so the least-privileged application role can revoke any instance and clear its own revocation (§9). Column-level grants — `UPDATE (last_seen_at)` to application roles with `revoked_at` reserved to an operator role — would make eviction database-enforced, but they need per-instance roles the first provider cannot create, so the first deployment either accepts the recorded limitation or does not rely on revocation as a security boundary.
