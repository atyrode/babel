...
import {
  TranscriptMapSourceSchema, TranscriptMapSegmentationSchema, TranscriptMapSpanSchema,
  TranscriptMapNodeSchema, type TranscriptMapCapture, type TranscriptMapSource,
  type TranscriptMapSegmentation, type TranscriptMapSpan, type TranscriptMapNode,
} from "./contract.ts";

const hash = (value: unknown): string => createHash("sha256").update(JSON.stringify(value)).digest("hex");
export function transcriptMapCaptureId(capture: Omit<TranscriptMapCapture, "id"> | TranscriptMapCapture): string {
  return `tmcap_${hash([capture.host, capture.harness, capture.session, capture.snapshot, capture.path])}`;
}
export function transcriptMapPlanId(source: TranscriptMapSource, segmentation: TranscriptMapSegmentation): string {
  return `tmplan_${hash([TranscriptMapSourceSchema.parse(source), TranscriptMapSegmentationSchema.parse(segmentation)])}`;
}
export function transcriptMapNodeId(planId: string, level: number, ordinal: number, span: TranscriptMapSpan, children: readonly string[], gap: TranscriptMapNode["gap"]): string {
