...
/*
  ...
  Reading (`read.ts`), ruling (`acts.ts`), suggesting (`suggest.ts`), rendering out
  (`export.ts`), starting (`launch.ts`) and draining (`drain.ts`) are the six groups — a reader
  of the roster should meet the answers before the acts, and the doors that spend money last.
  Rendering out is beside the reads because it writes nothing and PUBLISHES nothing: an export is
  a file the operator takes (§4.6), and the door holds no authority to reach a destination
  itself. Suggesting sits beside the acts because it is the one write that is NOT the operator's:
  a plugin he allow-listed proposes, and he still answers through `decide` (#410).
