...
import {
  ...
  TranscriptMapLocateInputSchema,
  TranscriptMapLocateReplySchema,
  TranscriptMapNativeReplySchema,
  TranscriptMapReadInputSchema,
  TranscriptMapReadReplySchema,
  TranscriptMapRegenerateInputSchema,
  TranscriptMapRegenerateReplySchema,
  TranscriptMapSourceInputSchema,
  ...
  type TranscriptMapReadRequest,
  type TranscriptMapReadResult,
  type TranscriptMapReadTrace,
  type TranscriptMapTarget,
...
class Interrupted extends Error {
  constructor(readonly state: Exclude<TranscriptMapNativeReply["state"], "complete">) {
    super(state);
  }
}
...
export function transcriptMapDoors(store: BabelStore): readonly Door[] {
  ...
  async function execute(ctx: GuestCtx, input: Input) {
    ...
    try {
      ...
      if (
        ...
        return {
          refused:
            "Transcript maps require the exact native map class and, for regeneration, operator write authority.",
        };
      const { target, requestId, ...request } = input;
      held = await traces.begin({
        principal: ctx.auth.principal.id,
        traceId: ctx.traceId,
        target,
        revision,
      ...
      async function native(
        ...
        source = false,
      ): Promise<{ result: TranscriptMapNativeResult; request: TranscriptMapNativeRequest }> {
        ...
        try {
          ...
          if (Buffer.byteLength(JSON.stringify(body)) > RECALL_MAX_REQUEST_BODY_BYTES)
          ...
          if (
            reply.requestId !== step.id ||
            (reply.result && reply.result.operation !== step.request.kind) ||
            Buffer.byteLength(JSON.stringify(reply)) > RECALL_MAX_RESULT_BYTES
          )
            throw new Interrupted("failed");
        } catch (error) {
        ...
        if (
          !reply.result ||
          (!source && reply.result.refusal !== null) ||
          !reply.result.context ||
          `map.${reply.result.context.classId}` !== target.operationId
        )
          throw new Interrupted("failed");
      ...
      const referenceInput =
        input.operation === "source"
          ? input
          : input.operation === "read" && "versionId" in input.request
            ? input.request
            : null;
      ...
      if (
        ...
        (input.request.kind === "status" ||
          (input.request.kind === "coverage" && input.request.captureId === undefined))
      ) {
        const inventory = await native("inventory", {
          kind: "map-inventory",
          maxCaptures: TRANSCRIPT_MAP_MAX_CAPTURES,
        });
        ...
        captures = [
          TranscriptMapCaptureSchema.parse({
            id: requestedReference.source.id,
            host: requestedReference.source.host,
            harness: requestedReference.source.harness,
            session: requestedReference.source.session,
            snapshot: requestedReference.source.snapshot,
            path: requestedReference.source.path,
            capturedAt: requestedReference.source.capturedAt,
          }),
        ];
      } else {
        captures = await maps.candidates(
          target.machineId,
          input.operation === "regenerate"
            ? { captureId: input.captureId }
            : input.operation === "source"
              ? { versionId: input.versionId, nodeId: input.nodeId }
              : input.request.kind === "search"
                ? { query: input.request.query }
                : input.request.kind === "coverage"
                  ? {
                      ...(input.request.captureId === undefined
                        ? {}
                        : { captureId: input.request.captureId }),
                    }
                  : "versionId" in input.request
                    ? { versionId: input.request.versionId, nodeId: input.request.nodeId }
                    : {},
        );
      ...
      while (captures.length > 0) {
        const body = {
          request: JSON.stringify({
            requestId: owned.id,
            request: { kind: "map-authorize", captures },
          }),
        };
        if (
          RecallServiceBodySchema.safeParse(body).success &&
          Buffer.byteLength(JSON.stringify(body)) <= RECALL_MAX_REQUEST_BODY_BYTES
        )
          break;
        captures.pop();
      ...
      const entries = captures.flatMap((capture) => {
        const access = authorized.result.accesses.find(
          (item) =>
            item.captureId === capture.id &&
            item.contextDigest === context.digest &&
            item.sensitivity <= context.ceiling,
        );
      ...
      if (input.operation === "source") {
        if (
          !requestedReference ||
          !entries.some((entry) => entry.capture.id === requestedReference.source.id)
        )
          throw new Interrupted("failed");
        const span = await native(
          "source",
          {
            kind: "map-span",
            source: requestedReference.source,
            span: requestedReference.node.span,
        ...
        if (sourceResult.refusal === null) {
          if (
            !returned ||
            JSON.stringify(returned.source) !== JSON.stringify(requestedReference.source) ||
            JSON.stringify(returned.span) !== JSON.stringify(requestedReference.node.span) ||
            returned.excerpt.bytes > input.maxBytes ||
            returned.excerpt.bytes > returned.span.byteLength ||
            returned.excerpt.truncated !== returned.excerpt.bytes < returned.span.byteLength ||
            returned.excerpt.bytes !== Buffer.byteLength(returned.excerpt.text) ||
            returned.excerpt.maxBytes !== input.maxBytes
          )
            throw new Interrupted("failed");
      ...
      const scope = {
        ...
        context: current,
        captureIds: entries.map((entry) => entry.capture.id),
      };
      await maps.recordAccess({ ...scope, entries, now: new Date().toISOString() });
      if (input.operation === "regenerate") {
        if (
          !entries.some((entry) => entry.capture.id === input.captureId) ||
          !(await ctx.auth.allows("containers:write"))
        )
          throw new Interrupted("failed");
        // The durable intent binds the original reason digest. No potentially secret reason is stored.
        const generation = await maps.regenerate({
          captureId: input.captureId,
          requestId: owned.id,
          reason: `Explicit map regeneration request ${owned.id}`,
          now: new Date().toISOString(),
        });
        const reply = TranscriptMapRegenerateReplySchema.parse({
          requestId: owned.id,
          state: "complete",
          generation,
        });
      ...
      if (input.operation === "source") {
        ...
        await traces.outcome(owned.id, round, {
          ...
          ...(span === undefined
            ...
            : {
                source: {
                  captureId: span.source.id,
                  span: span.span,
                  servedBytes: span.excerpt.bytes,
                  truncated: span.excerpt.truncated,
                },
              }),
      ...
      } else if (read.kind === "ancestors")
        views = await maps.ancestors(scope, read.versionId, read.nodeId);
      const historical = referenceInput
        ? (views[0] ?? (await maps.node(scope, referenceInput.versionId, referenceInput.nodeId)))
        : null;
      const coverage = historical
        ? { ...historical.coverage }
        : await maps.coverage(
            scope,
            read.kind === "coverage" ? read.captureId : requestedReference?.source.id,
          );
      // Historical version coverage must not be replaced by the current head's status.
      for (const view of views) {
        coverage.stale ||= view.coverage.stale;
        coverage.partial ||= view.coverage.partial;
      }
      if (inventoryPartial) {
        coverage.partial = true;
        coverage.tailBytes = null;
      }
      const result: TranscriptMapReadResult = {
        operation: read.kind,
        inference: true,
        views: views.map(
          ({ inference: _inference, coverage: _coverage, node, summary, ...view }) => {
            const { planId: _planId, ...compact } = node;
            return { ...view, node: compact, ...(summary === null ? {} : { summary }) };
          },
        ),
        coverage,
        status: await maps.status(scope),
        nextOffset:
          read.kind === "children" && read.offset + views.length < childCount
            ? read.offset + views.length
            : null,
      };
      ...
      while (
        Buffer.byteLength(JSON.stringify(reply)) > RECALL_MAX_RESULT_BYTES &&
        reply.result!.views.length > 0
      ) {
        reply.result!.views.pop();
        reply.result!.coverage.partial = true;
        reply.result!.coverage.tailBytes = null;
        if (read.kind === "children")
          reply.result!.nextOffset = read.offset + reply.result!.views.length;
      }
      if (
        Buffer.byteLength(JSON.stringify(reply)) > RECALL_MAX_RESULT_BYTES ||
        (views.length > 0 && reply.result!.views.length === 0)
      )
        throw new Interrupted("failed");
      const final = TranscriptMapReadReplySchema.parse(reply);
      const outcome: TranscriptMapReadTrace = {
        state: "complete",
        summaries: final.result!.views.map((view) => ({
          versionId: view.versionId,
          nodeId: view.node.id,
          summaryId: view.summary?.id ?? null,
        })),
      };
      ...
      try {
        await traces.outcome(held.id, round, { state, summaries: [] });
      } catch {
        return { requestId: held.id, state: "unavailable" as const };
      }
  ...
  return [
    defineDoor(
      defineServerAction({
        ...READ,
        name: ACTIONS.mapRead,
        title: "Navigate authorized immutable transcript maps",
        input: TranscriptMapReadInputSchema,
        result: TranscriptMapReadReplySchema,
        resultProjection: TRANSCRIPT_MAP_READ_RESULT_PROJECTION,
      }),
      async (ctx, input) => {
        const reply = await execute(ctx, { ...input, operation: "read" });
        return "refused" in reply && typeof reply.refused === "string"
          ? { refused: reply.refused }
          : TranscriptMapReadReplySchema.parse(reply);
      },
    ),
    defineDoor(
      defineServerAction({
        ...READ,
        name: ACTIONS.mapSource,
        title: "Read a map node's bounded canonical source",
        input: TranscriptMapSourceInputSchema,
        result: TranscriptMapNativeReplySchema,
        resultProjection: TRANSCRIPT_MAP_RESULT_PROJECTION,
      }),
      async (ctx, input) => {
        const reply = await execute(ctx, { ...input, operation: "source" });
        return "refused" in reply && typeof reply.refused === "string"
          ? { refused: reply.refused }
          : TranscriptMapNativeReplySchema.parse(reply);
      },
    ),
    defineDoor(
      defineServerAction({
        ...READ,
        name: ACTIONS.mapLocate,
        title: "Locate this caller's durable map request",
        input: TranscriptMapLocateInputSchema,
        result: TranscriptMapLocateReplySchema,
        resultProjection: TRANSCRIPT_MAP_LOCATE_RESULT_PROJECTION,
      }),
      async (ctx, { target, ...reference }) => {
        try {
          ...
          if (revision !== owned.revision) {
            await traces.outcome(owned.id, await traces.round(owned.id), {
              state: "expired",
              summaries: [],
            });
          ...
          return { refused: "Transcript map request could not be located or authorized." };
        }
      },
    ),
    defineDoor(
      defineServerAction({
        // Workspace writes are admitted here; execute separately checks the exact map grant.
        caps: ["containers:write"],
        delegates: READ.delegates,
        trace: READ.trace,
        name: ACTIONS.regenerateMap,
        title: "Request bounded explicit transcript map regeneration",
        input: TranscriptMapRegenerateInputSchema,
        result: TranscriptMapRegenerateReplySchema,
        resultProjection: REGENERATE_PROJECTION,
      }),
      async (ctx, input) => {
        const reply = await execute(ctx, { ...input, operation: "regenerate" });
        return "refused" in reply && typeof reply.refused === "string"
          ? { refused: reply.refused }
          : TranscriptMapRegenerateReplySchema.parse(reply);
      },
    ),
  ];
}
