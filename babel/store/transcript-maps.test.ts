...
import {
  ...
  TranscriptMapLocateReplySchema,
  TranscriptMapNativeReplySchema,
  TranscriptMapReadReplySchema,
  TranscriptMapRegenerateReplySchema,
...
const policy = TranscriptMapPolicySchema.parse({
  ...
  reviewRecipe: "mapping-review",
  recipes: [
    { id: "mapping-generate", version: 1, body: "Describe the supplied navigation span." },
...
const context: TranscriptMapContext = {
  digest: hash("inventory and classifications"),
  policyDigest: hash("classifications"),
  classId: "private",
  ceiling: 2,
  eligibleCaptures: 1,
  observedAt: NOW,
...
function fixture(
  ...
  path = "sessions/map-session.jsonl",
): MapFixture {
  const records = Array.from(
    { length: count },
    (_, index) => `${String(index).padStart(4, "0")} ${"x".repeat(1018)}\n`,
  );
  const capture = {
    host: "archive-host",
  ...
  const source = TranscriptMapSourceSchema.parse({
    ...
    captureDigest: hash(records.join("")),
    sourceDigest: hash(records.join("")),
    bytes: records.length * 1024,
  ...
  function span(first: number, last: number) {
    return {
      firstRecord: first + 1,
      lastRecord: last + 1,
      byteOffset: first * 1024,
      byteLength: (last - first + 1) * 1024,
      digest: hash(records.slice(first, last + 1).join("")),
      anchor: {
        line: first + 1,
        byteOffset: first * 1024,
        byteLength: 1024,
        digest: hash(records[first] ?? ""),
        time: null,
      },
    };
  }
  const nodes: TranscriptMapNode[] = records.map((_, index) => {
    ...
    return TranscriptMapNodeSchema.parse({
      id: transcriptMapNodeId(planId, 0, index, bounds, [], null),
      planId,
      parentId: null,
      level: 0,
      ordinal: index,
  ...
  while (layer.length > 1) {
    ...
    for (let start = 0; start < layer.length; start += 64) {
      ...
      const parent = TranscriptMapNodeSchema.parse({
        id: transcriptMapNodeId(planId, level, parents.length, bounds, ids, null),
        planId,
        parentId: null,
        level,
        ordinal: parents.length,
        span: bounds,
        children: ids,
        gap: null,
      });
...
async function settle(
  db: TestStore,
  maps: TranscriptMaps,
  item: TranscriptMapWork,
  result: TranscriptMapModelResult,
  now = NOW,
) {
  const runId = `run-${item.id}`;
  expect(await maps.startWork(item.id, { id: `claim-${item.id}`, runId, fence: 1 }, now)).toBe(
    true,
  );
  const details = await maps.work(item.id);
  if (!details) throw new Error("missing fixture work");
  const settlement = await maps.settlementStatements({
    details,
    result,
    runId,
    now,
  ...
  for (let pass = 0; pass < 5; pass += 1) {
    ...
    for (const item of await maps.offers(policy, NOW, 64)) {
      if (item.mode === "generate")
        await settle(db, maps, item, {
          kind: "summary",
          text: `Navigation for span ${item.nodeId.slice(-6)}.`,
        });
    }
  }
}
...
test("paged plans stay invisible until manifest verification and never authorize another class", async () => {
  ...
  const denied = {
    ...scope,
    context: { ...context, classId: "public", ceiling: 0, eligibleCaptures: 0 },
  };
...
test("a receipt larger than the engine batch limit publishes without truncating its graph", async () => {
  ...
  expect(parents.map((view) => view.node.span.byteLength)).toEqual([
    65536, 65536, 65536, 65536, 45056,
  ]);
...
test("a corrupt final manifest cannot publish previously ingested nodes", async () => {
  ...
  await expect(
    db.maps.recordPlan({ ...scope, ...corrupt, offset: 0, nextOffset: null, now: NOW }),
  ).rejects.toThrow();
  await expect(db.maps.ensureVersion(data.plan.id, policy, NOW)).rejects.toThrow();
...
test("identical captures and unchanged prefix spans bind original prose without buying another run", async () => {
  ...
  expect((await db.maps.node(scope, v3.id, extended.nodes[0]?.id ?? ""))?.summary?.id).toBe(
    original?.summary?.id,
  );
...
test("only actual serving buys a bounded review, including when artifacts have many bindings", async () => {
  ...
  await settle(db, db.maps, review, {
    kind: "review",
    verdict: "correct",
    reason: "The span has a missing qualification.",
  });
  await db.maps.refreshWork(policy, NOW, 128);
  ...
  const correctedId = await settle(db, db.maps, correction, {
    kind: "summary",
    text: "Navigation with the qualification preserved.",
  });
  if (correctedId === null) throw new Error("The correction did not publish a summary.");
  await db.maps.refreshWork(policy, NOW, 128);
  const next = await db.maps.offers(policy, NOW);
  expect(next.some((item) => item.mode === "review" && item.baseSummaryId === correctedId)).toBe(
    false,
  );
  expect((await db.maps.node(scope, v2.id, repeat.nodes[0]?.id ?? ""))?.summary?.id).toBe(
    correctedId,
  );
  expect((await db.maps.node(scope, v1.id, first.plan.rootId ?? ""))?.coverage.stale).toBe(true);
  for (const item of next)
    if (item.mode === "correct")
      await settle(db, db.maps, item, { kind: "summary", text: "Updated parent navigation." });
  await db.maps.refreshWork(policy, NOW, 128);
  ...
  expect(
    (await db.maps.offers(policy, LATER)).some(
      (item) => item.baseSummaryId === summaryId && item.mode === "review",
    ),
  ).toBe(false);
});
...
test("refused output is never stored, attempts back off, and false fences write no artifact", async () => {
  ...
  await expect(
    db.maps.settlementStatements({
      details,
      runId,
      now: NOW,
      guard: { sql: "1", params: [] },
      result: { kind: "summary", text: `credential: ${"gh" + "p_"}${"a1B2c3D4".repeat(5)}` },
    }),
  ).rejects.toThrow();
  const fenced = await db.maps.settlementStatements({
    details,
    runId,
    now: NOW,
    guard: { sql: "0", params: [] },
    result: { kind: "summary", text: "Safe navigation." },
  });
  await db.db.batch(fenced.statements);
  expect((await db.maps.coverage(scope)).summarizedBytes).toBe(0);
  await db.db.batch(
    await db.maps.failureStatements({
      workId: item.id,
      now: NOW,
      reason: "raw rejected provider output",
      guard: { sql: "1", params: [] },
    }),
  );
  expect(await db.maps.offers(policy, NOW)).toEqual([]);
  ...
  const rows = await db.db.query<{ payload: string; reason: string }>(
    "SELECT payload,reason FROM transcript_map_work WHERE id=?",
    [item.id],
  );
...
test("explicit regeneration is idempotent and preserves the previous producing version", async () => {
  ...
  const request = {
    captureId: data.plan.source.id,
    requestId: "regenerate-once",
    reason: "Revisit this capture.",
    now: LATER,
  };
  ...
  expect((await db.maps.node(scope, original.id, data.plan.rootId ?? ""))?.summary).toEqual(
    before?.summary,
  );
  expect((await db.maps.node(scope, original.id, data.plan.rootId ?? ""))?.coverage.stale).toBe(
    true,
  );
  await db.maps.refreshWork(policy, LATER);
...
test("oversized records remain explicit gaps instead of becoming paid work or silent coverage", async () => {
  ...
  const item = TranscriptMapNodeSchema.parse({
    id: transcriptMapNodeId(planId, 0, 0, span, [], "record-too-large"),
    planId,
    parentId: null,
    level: 0,
    ordinal: 0,
    span,
    children: [],
    gap: "record-too-large",
  });
  const data: MapFixture = {
    plan: {
      ...first.plan,
      id: planId,
      source,
      rootId: item.id,
      gapBytes: 2048,
      digest: transcriptMapManifestDigest([item]),
    },
    nodes: [item],
    access: first.access,
  };
...
test("read attestations preserve worker inventory progress and planning resumes the first missing page", async () => {
  ...
  await db.maps.recordCatalog({
    ...scope,
    entries: [{ capture, access: data.access }],
  ...
  await db.maps.recordAccess({
    ...scope,
    context: { ...context, observedAt: LATER },
    entries: [],
    now: LATER,
  });
...
function reader(db: TestStore, data: MapFixture) {
  ...
  const capture = TranscriptMapCaptureSchema.parse({
    id: data.plan.source.id,
    host: data.plan.source.host,
    harness: data.plan.source.harness,
    session: data.plan.source.session,
    snapshot: data.plan.source.snapshot,
    path: data.plan.source.path,
    capturedAt: data.plan.source.capturedAt,
  });
  ...
  const ctx = (traceId: number, principal = "reader") =>
    ({
      ...
      auth: {
        ...
        allows: async (cap: string, selected?: { operationId?: string }) =>
          cap === "containers:write"
            ? control.write
            : selected?.operationId === "private" ||
              (control.allowed && selected?.operationId === "map.private"),
      },
      services: {
        ...
        invokeInstance: async (args: {
          ...
          if (args.serviceId !== RECALL_SERVICE_ID || args.expectedRevision !== control.revision)
            return { ok: false };
          const frame = ArchiveServiceRequestSchema.parse(JSON.parse(args.input.request));
          if (frame.request.kind === "poll")
            return {
              ok: true,
              result: replies.get(frame.requestId) ?? {
                requestId: frame.requestId,
                state: "expired",
              },
            };
          if (posted.has(frame.requestId))
            throw new Error("A dispatched native request was reposted");
          ...
          const result = {
            ...
            context,
            entries:
              request.kind === "map-inventory" && control.attest
                ? [{ capture, access: data.access }]
                : [],
            nextCursor: null,
            accesses: request.kind === "map-authorize" && control.attest ? [data.access] : [],
            cost: {
              fetchedFiles: 0,
              fetchedBytes: 0,
              cacheHits: 0,
            ...
            ...(request.kind !== "map-span" || control.refuseSource
              ? {}
              : {
                  span: {
                    ...
                    excerpt: {
                      ...
                      text: "x".repeat(
                        control.overboundSource ? request.maxBytes + 1 : request.maxBytes,
                      ),
                      bytes: control.overboundSource ? request.maxBytes + 1 : request.maxBytes,
                      maxBytes: request.maxBytes,
                      truncated: true,
                      firstRecord: request.span.firstRecord,
                      lastRecord: request.span.firstRecord,
                    },
                  },
                }),
          };
...
test("unmapped nodes remain navigable through the actual SDK result projection", async () => {
  ...
  const delivered = TranscriptMapReadReplySchema.parse(
    projectJson(
      reply,
...
test("map readers reauthorize candidates independently of worker inventory and only disclosed summaries become reviewable", async () => {
  ...
  const served = await db.db.query<{ summary_id: string }>(
    "SELECT summary_id FROM transcript_map_served",
  );
  expect(served.map((row) => row.summary_id)).toEqual([reply.result!.views[0]!.summary!.id]);
  f.control.attest = false;
  const revoked = TranscriptMapReadReplySchema.parse(
    await f.knock(ACTIONS.mapRead, { ...request, requestId: reply.requestId }, 103),
  );
...
test("source recovery binds caller, request and revision without reposting uncertain work or serving inference", async () => {
  ...
  const interrupted = TranscriptMapNativeReplySchema.parse(
    await f.knock(ACTIONS.mapSource, request, 201),
  );
  expect(interrupted.state).toBe("unavailable");
  const located = TranscriptMapLocateReplySchema.parse(
    await f.knock(ACTIONS.mapLocate, { target: f.target, traceId: 201 }, 202),
  );
  expect(located.requestId).toBe(interrupted.requestId);
  expect(
    await f.knock(
      ACTIONS.mapLocate,
      { target: f.target, requestId: located.requestId },
      203,
      "other-reader",
    ),
  ...
  expect(
    await f.knock(
      ACTIONS.mapSource,
      { ...request, maxBytes: 24, requestId: located.requestId },
      204,
    ),
  ).toHaveProperty("refused");
  const resumed = TranscriptMapNativeReplySchema.parse(
    await f.knock(ACTIONS.mapSource, { ...request, requestId: located.requestId }, 205),
  );
  ...
  expect(await db.db.query("SELECT summary_id FROM transcript_map_served")).toEqual([]);
  const replay = TranscriptMapNativeReplySchema.parse(
    await f.knock(ACTIONS.mapSource, { ...request, requestId: located.requestId }, 206),
  );
  expect(replay.result?.span?.excerpt).toEqual(resumed.result?.span?.excerpt);
  const costs = await db.db.query<{ n: number }>(
    "SELECT count(*) n FROM transcript_map_read_outcomes WHERE json_extract(outcome,'$.cost') IS NOT NULL",
  );
  expect(Number(costs[0]?.n)).toBe(1);
  f.control.revision = "revision-2";
  expect(
    await f.knock(ACTIONS.mapSource, { ...request, requestId: located.requestId }, 207),
  ).toEqual({ requestId: located.requestId, state: "expired" });
});
...
test("overbound source and an atomic disclosure failure cannot create served-summary eligibility", async () => {
  ...
  const source = TranscriptMapNativeReplySchema.parse(
    await f.knock(
      ACTIONS.mapSource,
      { target: f.target, versionId: version.id, nodeId: data.nodes[0]!.id, maxBytes: 32 },
      301,
    ),
  );
  expect(source.state).toBe("failed");
  await db.db
    .run(`CREATE TRIGGER synthetic_disclosure_failure BEFORE INSERT ON transcript_map_served
    BEGIN SELECT RAISE(ABORT,'synthetic durable write failure'); END`);
  const read = TranscriptMapReadReplySchema.parse(
    await f.knock(
      ACTIONS.mapRead,
      {
        target: f.target,
  ...
  expect(
    await db.db.query(
      "SELECT outcome FROM transcript_map_read_outcomes WHERE json_extract(outcome,'$.state')='complete'",
    ),
  ).toEqual([]);
});
...
test("bounded child disclosure advances only over included items and retains historical producing provenance", async () => {
  ...
  await db.maps.regenerate({
    captureId: data.plan.source.id,
    requestId: "newer-generation",
    reason: "Update",
    now: LATER,
  });
  await db.maps.ensureVersion(data.plan.id, policy, LATER);
  const f = reader(db, data);
  const reply = TranscriptMapReadReplySchema.parse(
    await f.knock(
      ACTIONS.mapRead,
      {
        target: f.target,
        request: { kind: "children", versionId: version.id, nodeId: data.plan.rootId!, limit: 16 },
      },
      401,
    ),
  );
  ...
  const served = await db.db.query<{ summary_id: string }>(
    "SELECT summary_id FROM transcript_map_served ORDER BY summary_id",
  );
  expect(served.map((row) => row.summary_id)).toEqual(
    result.views.map((view) => view.summary!.id).sort(),
  );
});
...
test("explicit regeneration requires both grants, remains idempotent and never stores secret-bearing queries or reasons", async () => {
  ...
  const first = TranscriptMapRegenerateReplySchema.parse(
    await f.knock(ACTIONS.regenerateMap, request, 502),
  );
  const repeated = TranscriptMapRegenerateReplySchema.parse(
    await f.knock(ACTIONS.regenerateMap, request, 503),
  );
  expect(first.generation).toBe(1);
  expect(repeated.generation).toBe(1);
  expect(
    await f.knock(ACTIONS.regenerateMap, { ...request, reason: "Changed intent" }, 504),
  ).toHaveProperty("refused");
  await f.knock(
    ACTIONS.mapRead,
    { target: f.target, request: { kind: "search", query: secret } },
    505,
  );
...
test("native source refusal stays explicit while bounded coverage never counts unattested captures", async () => {
  ...
  expect(await db.db.query("SELECT summary_id FROM transcript_map_served")).toEqual([]);
  f.control.attest = false;
  const status = TranscriptMapReadReplySchema.parse(
    await f.knock(ACTIONS.mapRead, { target: f.target, request: { kind: "status" } }, 602),
  );
  expect(status.result?.status).toMatchObject({ verifiedMappedCaptures: 0, partial: true });
  const coverage = TranscriptMapReadReplySchema.parse(
    await f.knock(ACTIONS.mapRead, { target: f.target, request: { kind: "coverage" } }, 603),
  );
  expect(coverage.result?.coverage).toMatchObject({
    sourceBytes: 0,
    partial: true,
    tailBytes: null,
  });
