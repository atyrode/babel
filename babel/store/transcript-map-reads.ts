...
export function transcriptMapReads(store: TranscriptMapStore) {
  async function locate(
    principal: string,
    reference: { requestId?: string | undefined; traceId?: number | undefined },
  ): Promise<MapReadRequest | null> {
    const rows = await store.db.query<{
      id: string;
      operation: MapReadRequest["operation"];
      target: string;
    ...
    return row
      ? {
          ...
          target: TranscriptMapTargetSchema.parse(JSON.parse(row.target)),
          revision: row.service_revision,
  ...
  async function begin(input: {
    ...
    await store.db.run(
      `INSERT OR IGNORE INTO transcript_map_requests
    ...
    const held = await locate(
      input.principal,
      input.requestId === undefined ? { traceId: input.traceId } : { requestId: id },
    );
    if (
      !held ||
      held.operation !== input.operation ||
      held.digest !== requestDigest ||
      JSON.stringify(held.target) !== JSON.stringify(target)
    )
      throw new Error("Map request identity mismatch");
    return held;
  }
  async function round(requestId: string): Promise<number> {
    const rows = await store.db.query<{ n: number }>(
      `SELECT coalesce(max(round)+1,0) n FROM transcript_map_read_outcomes
      WHERE request_id=? AND json_extract(outcome,'$.state')='complete'`,
      [requestId],
    );
  ...
  async function native(
    ...
    await store.db.run(
      `INSERT OR IGNORE INTO transcript_map_native_requests
      (request_id,round,stage,native_id,request,created_at) VALUES(?,?,?,?,?,?)`,
      [requestId, round, stage, nativeId, JSON.stringify(parsed), now()],
    );
    const rows = await store.db.query<{ native_id: string; request: string }>(
      `SELECT native_id,request
      FROM transcript_map_native_requests WHERE request_id=? AND round=? AND stage=?`,
      [requestId, round, stage],
    );
    const row = rows[0];
    if (!row) throw new Error("Map native intent unavailable");
    return {
      id: row.native_id,
      fresh: row.native_id === nativeId,
  ...
  async function outcome(
    requestId: string,
    round: number,
    input: TranscriptMapReadTrace,
  ): Promise<void> {
    const trace = TranscriptMapReadTraceSchema.parse(input);
    const summaryIds =
      trace.state === "complete" && trace.source === undefined
        ? [
            ...new Set(
              trace.summaries.flatMap((item) => (item.summaryId === null ? [] : [item.summaryId])),
            ),
          ]
        : [];
    const at = now();
