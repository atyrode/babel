...
export interface TranscriptMaps {
  ...
  nextPlan(machineId: string, segmentation: TranscriptMapSegmentation): Promise<TranscriptMapNextPlan | null>;
  recordPlan(input: TranscriptMapPlanInput): Promise<{ complete: boolean }>;
  ensureVersion(planId: string, policy: TranscriptMapPolicy, now: string, generation?: number): Promise<TranscriptMapVersion>;
  ...
  ancestors(scope: TranscriptMapScope, versionId: string, nodeId: string): Promise<TranscriptMapView[]>;
  ...
  regenerate(input: { captureId: string; requestId: string; reason: string; now: string }): Promise<number>;
}
...
const digest = (value: unknown): string => `sha256:${createHash("sha256").update(json(value)).digest("hex")}`;
...
function recipes(policy: TranscriptMapPolicy) {
  const generate = policy.recipes.find((recipe) => recipe.id === policy.generateRecipe && recipe.enabled !== false);
  const review = policy.recipes.find((recipe) => recipe.id === policy.reviewRecipe && recipe.enabled !== false);
...
function emptyCoverage(): TranscriptMapCoverage {
  return { sourceBytes: 0, summarizedBytes: 0, directBytes: 0, unmappedBytes: 0, gapBytes: 0, levels: [], partial: false, stale: false, tailBytes: 0 };
}
...
export function transcriptMaps(store: TranscriptMapStore): TranscriptMaps {
  ...
  async function one<T>(table: string, id: string): Promise<T | null> {
    const rows = await db.query<{ payload: string }>(`SELECT payload FROM ${table} WHERE id=?`, [id]);
    return rows[0] ? JSON.parse(rows[0].payload) as T : null;
  }
  async function batch(statements: readonly SqlStatement[]): Promise<void> {
    for (let start = 0; start < statements.length; start += PAGE) await db.batch(statements.slice(start, start + PAGE));
  ...
  async function rememberContext(scope: TranscriptMapScope, nextCursor: string | null, preserveCursor: boolean, mapping: boolean, now: string): Promise<void> {
    const context = TranscriptMapContextSchema.parse(scope.context);
    const newer = await db.query<{ n: number }>(`SELECT count(*) n FROM transcript_map_contexts WHERE machine_id=? AND observed_at>? AND digest!=?`, [scope.machineId, context.observedAt, context.digest]);
    ...
    await db.batch([
      ...
      { sql: `INSERT INTO transcript_map_contexts(machine_id,class_id,digest,ceiling,observed_at,payload,next_cursor,cataloged_at,completed_at,mapping,mapping_payload)
        ...
        params: [scope.machineId, context.classId, context.digest, context.ceiling, context.observedAt, json(context),
          nextCursor, preserveCursor ? null : now, !preserveCursor && nextCursor === null ? now : null,
          mapping ? 1 : 0, mapping ? json(context) : null, preserveCursor ? 1 : 0, preserveCursor ? 1 : 0] },
    ]);
  }
  async function recordCatalog(input: TranscriptMapCatalogInput, preserveCursor = false, mapping = true): Promise<void> {
    if (input.entries.length > TRANSCRIPT_MAP_MAX_CAPTURES) throw new Error("transcript catalog page exceeds bound");
    ...
    for (const entry of input.entries) {
      ...
      if (capture.id !== transcriptMapCaptureId(capture) || access.captureId !== capture.id ||
        access.contextDigest !== input.context.digest || access.sensitivity > input.context.ceiling) throw new Error("invalid transcript map access attestation");
      ...
      statements.push({ sql: `INSERT OR IGNORE INTO transcript_map_captures(id,host,harness,session,captured_at,payload) VALUES(?,?,?,?,?,?)`, params: [capture.id, capture.host, capture.harness, capture.session, capture.capturedAt, json(capture)] });
      statements.push({ sql: `INSERT INTO transcript_map_access(machine_id,capture_id,context_digest,sensitivity) VALUES(?,?,?,?)
        ON CONFLICT(machine_id,capture_id,context_digest) DO UPDATE SET sensitivity=max(sensitivity,excluded.sensitivity)`, params: [input.machineId, capture.id, access.contextDigest, access.sensitivity] });
    }
  ...
  async function catalogState(machineId: string): Promise<TranscriptMapCatalogState> {
    const rows = await db.query<{ payload: string; next_cursor: string | null; completed_at: string | null }>(
      `SELECT mapping_payload payload,next_cursor,completed_at FROM transcript_map_contexts WHERE machine_id=? AND mapping=1
       ORDER BY cataloged_at DESC,observed_at DESC,class_id LIMIT 1`, [machineId]);
    return { context: rows[0] ? TranscriptMapContextSchema.parse(JSON.parse(rows[0].payload)) : null,
      nextCursor: rows[0]?.next_cursor ?? null, completedAt: rows[0]?.completed_at ?? null };
  }
  async function nextPlan(machineId: string, segmentation: TranscriptMapSegmentation): Promise<TranscriptMapNextPlan | null> {
    ...
    const rows = await db.query<{ payload: string; next_offset: number }>(`SELECT c.payload,
  ...
  async function recordPlan(input: TranscriptMapPlanInput): Promise<{ complete: boolean }> {
    ...
    if (plan.id !== transcriptMapPlanId(plan.source, plan.segmentation)) throw new Error("invalid transcript plan identity");
    const { coordinates: _coordinates, captureDigest: _captureDigest, sourceDigest: _sourceDigest, bytes: _bytes, records: _records, ...capture } = plan.source;
    await recordCatalog({ ...input, entries: [{ capture, access: input.access }], nextCursor: null }, true);
    ...
    if (!Number.isSafeInteger(input.offset) || input.offset < 0 || input.nodes.length > TRANSCRIPT_MAP_JOB_PAGE_NODES ||
      input.offset + input.nodes.length > plan.nodeCount ||
      (input.nextOffset === null ? input.offset + input.nodes.length !== plan.nodeCount :
        input.nextOffset !== input.offset + input.nodes.length || input.nextOffset >= plan.nodeCount || !input.nodes.length)) throw new Error("invalid transcript plan page");
    await db.run(`INSERT OR IGNORE INTO transcript_map_plans(id,capture_id,payload,created_at) VALUES(?,?,?,?)`, [plan.id, plan.source.id, json(plan), input.now]);
    const statements: SqlStatement[] = [];
    for (let index = 0; index < input.nodes.length; index += 1) {
      const node = TranscriptMapNodeSchema.parse(input.nodes[index]);
      if (node.planId !== plan.id || node.level >= plan.segmentation.maxDepth ||
        node.id !== transcriptMapNodeId(plan.id, node.level, node.ordinal, node.span, node.children, node.gap) ||
        node.span.byteOffset + node.span.byteLength > plan.source.bytes || node.span.lastRecord > plan.source.records ||
        (node.gap !== null && node.children.length !== 0) || (node.children.length === 0 && node.gap === null && node.span.byteLength > plan.segmentation.leafBytes) ||
        (node.gap === "record-too-large" && (node.span.firstRecord !== node.span.lastRecord || node.span.byteLength <= plan.segmentation.leafBytes)) ||
        node.children.length > plan.segmentation.fanout) throw new Error("invalid transcript map node");
      statements.push({ sql: `INSERT OR IGNORE INTO transcript_map_nodes(id,plan_id,position,parent_id,level,ordinal,byte_offset,byte_length,gap,payload) VALUES(?,?,?,?,?,?,?,?,?,?)`,
        params: [node.id, plan.id, input.offset + index, node.parentId, node.level, node.ordinal, node.span.byteOffset, node.span.byteLength, node.gap, json(node)] });
    }
    await batch(statements);
  ...
  async function verifyPlan(plan: TranscriptMapPlan): Promise<void> {
    ...
    while (position < plan.nodeCount) {
      const rows = await db.query<{ payload: string; position: number }>(`SELECT payload,position FROM transcript_map_nodes WHERE plan_id=? AND position>=? ORDER BY position LIMIT ?`, [plan.id, position, PAGE]);
      ...
      for (const row of rows) {
        ...
        if (node.parentId === null) {
          roots += 1;
          if (node.id !== plan.rootId || node.span.byteOffset !== 0 || node.span.byteLength !== plan.source.bytes ||
            node.span.firstRecord !== 1 || node.span.lastRecord !== plan.source.records || node.span.digest !== plan.source.sourceDigest) throw new Error("invalid transcript root");
        } else {
          const parent = await one<TranscriptMapNode>("transcript_map_nodes", node.parentId);
          if (!parent || parent.planId !== plan.id || parent.level <= node.level || !parent.children.includes(node.id)) throw new Error("invalid transcript parent link");
        }
        if (!node.children.length) {
          ...
          for (const id of node.children) {
            ...
            if (!child || child.parentId !== node.id || child.planId !== plan.id || child.level >= node.level ||
              child.span.byteOffset !== byteEnd || child.span.firstRecord !== lastRecord + 1) throw new Error("noncontiguous transcript child span");
            byteEnd += child.span.byteLength;
            lastRecord = child.span.lastRecord;
          }
          if (byteEnd !== node.span.byteOffset + node.span.byteLength || lastRecord !== node.span.lastRecord) throw new Error("transcript children do not cover parent");
        }
      }
    }
    let after = -1;
    for (;;) {
      const leaves = await db.query<{ payload: string; byte_offset: number }>(`SELECT payload,byte_offset FROM transcript_map_nodes WHERE plan_id=? AND json_array_length(json_extract(payload,'$.children'))=0 AND byte_offset>? ORDER BY byte_offset LIMIT ?`, [plan.id, after, PAGE]);
    ...
    if (`sha256:${hash.update("]").digest("hex")}` !== plan.digest || gaps !== plan.gapBytes ||
      roots !== (plan.source.bytes === 0 ? 0 : 1) || leafEnd !== plan.source.bytes || recordEnd !== plan.source.records ||
      (plan.rootId === null) !== (plan.source.bytes === 0) || plan.direct !== (plan.source.bytes <= plan.segmentation.directBytes)) throw new Error("transcript plan manifest verification failed");
  }
  async function ensureVersion(planId: string, raw: TranscriptMapPolicy, now: string, generation?: number): Promise<TranscriptMapVersion> {
    ...
    if (held) {
      if (gen === Number(requests[0]?.generation ?? 0)) await db.run(`INSERT INTO transcript_map_heads(machine_id,plan_id,version_id)
        VALUES(?,?,?) ON CONFLICT(machine_id,plan_id) DO UPDATE SET version_id=excluded.version_id`, [policy.machineId, planId, held.id]);
      return held;
    }
    if (gen !== Number(requests[0]?.generation ?? 0)) throw new Error("cannot create an obsolete generation");
    const heads = await db.query<{ version_id: string }>(`SELECT version_id FROM transcript_map_heads WHERE machine_id=? AND plan_id=?`, [policy.machineId, planId]);
    const selected = recipes(policy);
    const version = TranscriptMapVersionSchema.parse({ id, planId, contractDigest: key, profile: policy.profile, generateRecipe: selected.generate, reviewRecipe: selected.review, generation: gen, supersedes: heads[0]?.version_id ?? null, createdAt: now });
    await db.batch([
      { sql: `INSERT OR IGNORE INTO transcript_map_versions(id,plan_id,machine_id,contract_digest,generation,payload,policy,created_at) VALUES(?,?,?,?,?,?,?,?)`, params: [id, planId, policy.machineId, key, gen, json(version), json(policy), now] },
      { sql: `INSERT INTO transcript_map_heads(machine_id,plan_id,version_id) VALUES(?,?,?) ON CONFLICT(machine_id,plan_id) DO UPDATE SET version_id=excluded.version_id`, params: [policy.machineId, planId, id] },
    ]);
    store.touch?.();
    return version;
  }

  async function boundSummary(versionId: string, nodeId: string, includeRejected = false): Promise<TranscriptMapSummary | null> {
    const rows = await db.query<{ payload: string }>(`SELECT s.payload FROM transcript_map_bindings b
      JOIN transcript_map_summaries s ON s.id=b.summary_id WHERE b.version_id=? AND b.node_id=?
      ${includeRejected ? "" : "AND NOT EXISTS (SELECT 1 FROM transcript_map_reviews r WHERE r.summary_id=s.id AND r.verdict IN ('correct','reject'))"}`, [versionId, nodeId]);
    return rows[0] ? TranscriptMapSummarySchema.parse(JSON.parse(rows[0].payload)) : null;
  }
  async function childInputs(versionId: string, parent: TranscriptMapNode): Promise<TranscriptMapChildSummary[] | null> {
    ...
    for (const id of parent.children) {
      ...
      inputs.push({ nodeId: id, summaryId: summary?.id ?? null, text: summary?.text ?? null, gap: child.gap });
    }
    return inputs;
  }
  function inputKey(plan: TranscriptMapPlan, item: TranscriptMapNode, version: TranscriptMapVersion, inputs: readonly TranscriptMapChildSummary[]): string {
    return digest([plan.source.host, plan.source.harness, plan.source.session,
      version.contractDigest, version.generation === 0 ? null : [plan.source.id, version.generation],
      item.level, item.span.digest, item.span.byteLength, item.span.lastRecord - item.span.firstRecord + 1,
  ...
  async function work(id: string): Promise<TranscriptMapWorkDetails | null> {
    ...
    const plan = version ? await one<TranscriptMapPlan>("transcript_map_plans", version.planId) : null;
    if (!version || !item || !plan) return null;
    const baseSummary = queued.baseSummaryId ? await one<TranscriptMapSummary>("transcript_map_summaries", queued.baseSummaryId) : null;
    const feedback = queued.mode === "correct" && queued.baseSummaryId ? await db.query<{ reason: string }>(
      `SELECT reason FROM transcript_map_reviews WHERE summary_id=? AND verdict IN ('correct','reject') ORDER BY created_at DESC,work_id DESC LIMIT 1`, [queued.baseSummaryId]) : [];
    const contexts = await db.query<{ payload: string }>(`SELECT c.mapping_payload payload FROM transcript_map_contexts c
      ...
      ORDER BY c.cataloged_at DESC,c.observed_at DESC,c.class_id LIMIT 1`, [version.id, plan.source.id]);
    return { work: TranscriptMapWorkSchema.parse(queued), plan, node: item, version, baseSummary, feedback: feedback[0]?.reason ?? null,
      context: contexts[0] ? TranscriptMapContextSchema.parse(JSON.parse(contexts[0].payload)) : null };
  }
  ...
  async function eligible(details: TranscriptMapWorkDetails, policy: TranscriptMapPolicy): Promise<boolean> {
    if (details.version.contractDigest !== contract(policy) || details.node.gap !== null || details.plan.direct ||
      details.work.attempt > policy.maxAttempts || details.work.correctionDepth > policy.maxCorrections) return false;
    if (details.work.mode === "review") {
      const count = await db.query<{ n: number }>(`SELECT count(*) n FROM transcript_map_work
  ...
  async function queue(details: Pick<TranscriptMapWorkDetails, "plan" | "node" | "version">, policy: TranscriptMapPolicy,
    ...
    key: string, servedSeq: number, now: string): Promise<number> {
    const id = identity("tmwork", [details.version.id, details.node.id, mode, base?.id ?? null, key, servedSeq]);
    const item = TranscriptMapWorkSchema.parse({ id, versionId: details.version.id, nodeId: details.node.id, mode,
      baseSummaryId: base?.id ?? null, children: inputs, correctionDepth: depth, attempt: 1, createdAt: now });
    let unique = mode === "generate"
    ...
    const params: SqlParam[] = [id, item.versionId, item.nodeId, mode, item.baseSummaryId, now, now, json(item), key, servedSeq];
    ...
    const result = await db.run(`INSERT OR IGNORE INTO transcript_map_work(id,version_id,node_id,mode,base_summary_id,
      attempt,state,ready_at,created_at,payload,input_key,served_seq) SELECT ?,?,?,?,?,1,'queued',?,?,?,?,? WHERE ${unique}`, params);
  ...
  async function refreshWork(raw: TranscriptMapPolicy, now: string, limit = 32): Promise<number> {
    ...
    const plans = await db.query<{ id: string }>(`SELECT p.id FROM transcript_map_plans p
      ...
      AND NOT EXISTS (SELECT 1 FROM transcript_map_heads h JOIN transcript_map_versions v ON v.id=h.version_id
        WHERE h.plan_id=p.id AND h.machine_id=? AND v.contract_digest=?
          AND v.generation=(SELECT coalesce(max(generation),0) FROM transcript_map_regenerations WHERE capture_id=p.capture_id))
      ORDER BY p.created_at,p.id LIMIT ?`, [policy.machineId, json(policy.segmentation), policy.machineId, key, Math.min(cap, 8)]);
    for (const row of plans) await ensureVersion(row.id, policy, now);
    const cursor = await db.query<{ version_cursor: number; node_cursor: number }>(`SELECT version_cursor,node_cursor FROM transcript_map_scan WHERE machine_id=? AND contract_digest=?`, [policy.machineId, key]);
    ...
    for (const row of rows) {
      ...
      const binding = await db.query<{ input_key: string }>(`SELECT input_key FROM transcript_map_bindings WHERE version_id=? AND node_id=?`, [version.id, item.id]);
      let boundKey = binding[0]?.input_key;
      const reusable = await db.query<{ id: string }>(`SELECT s.id FROM transcript_map_summaries s WHERE s.reuse_key=?
        AND NOT EXISTS (SELECT 1 FROM transcript_map_reviews r WHERE r.summary_id=s.id AND r.verdict IN ('correct','reject'))
        ORDER BY CAST(json_extract(s.payload,'$.correctionDepth') AS INTEGER) DESC,s.created_at DESC,s.id DESC LIMIT 1`, [reuseKey]);
      if (reusable[0] && reusable[0].id !== base?.id) {
        await db.run(`INSERT INTO transcript_map_bindings(version_id,node_id,summary_id,input_key) VALUES(?,?,?,?)
      ...
      if (!base) {
        const depth = Math.max(0, ...await Promise.all(inputs.map(async (child) =>
          child.summaryId ? (await one<TranscriptMapSummary>("transcript_map_summaries", child.summaryId))?.correctionDepth ?? 0 : 0)));
        if (depth <= policy.maxCorrections) added += await queue({ version, node: item, plan }, policy, "generate", null, inputs, depth, reuseKey, 0, now);
        continue;
      }
      const reviews = await db.query<{ verdict: string }>(`SELECT verdict FROM transcript_map_reviews WHERE summary_id=? ORDER BY created_at DESC,work_id DESC LIMIT 1`, [base.id]);
      if (boundKey !== reuseKey || reviews[0]?.verdict === "correct" || reviews[0]?.verdict === "reject") {
        if (base.correctionDepth < policy.maxCorrections) added += await queue({ version, node: item, plan }, policy, "correct", base, inputs, base.correctionDepth + 1, reuseKey, 0, now);
        continue;
      }
      const served = await db.query<{ seq: number }>(`SELECT coalesce(max(rowid),0) seq FROM transcript_map_served WHERE summary_id=?`, [base.id]);
      if (Number(served[0]?.seq ?? 0) > 0 && policy.maxReviews > 0) added += await queue({ version, node: item, plan }, policy, "review", base, inputs, base.correctionDepth, reuseKey, Number(served[0]?.seq), now);
    }
    const last = rows.length === cap ? rows[rows.length - 1] : undefined;
    await db.run(`INSERT INTO transcript_map_scan(machine_id,contract_digest,version_cursor,node_cursor) VALUES(?,?,?,?)
  ...
  async function offers(raw: TranscriptMapPolicy, now: string, limit = 32): Promise<TranscriptMapWork[]> {
    const policy = TranscriptMapPolicySchema.parse(raw);
    const rows = await db.query<{ id: string }>(`SELECT w.id FROM transcript_map_work w JOIN transcript_map_versions v ON v.id=w.version_id
      ...
      ORDER BY w.ready_at,w.created_at,w.id LIMIT ?`, [policy.machineId, contract(policy), now, policy.maxAttempts, MAX_SCAN]);
    ...
    for (const row of rows) {
      ...
      else await db.run(`UPDATE transcript_map_work SET state='obsolete' WHERE id=? AND state='queued'`, [row.id]);
  ...
  async function startWork(id: string, claim: { id: string; runId: string; fence: number }, now: string): Promise<boolean> {
    ...
    if (!rows[0] || !await eligible(details, TranscriptMapPolicySchema.parse(JSON.parse(rows[0].policy)))) return false;
    if (running) return true;
    const check = readiness(details);
    const changed = await db.run(`UPDATE transcript_map_work SET state='running',claim_id=?,run_id=?,fence=?
      WHERE id=? AND state='queued' AND ready_at<=? AND ${check.sql}`, [claim.id, claim.runId, claim.fence, id, now, ...check.params]);
    if (changed.changes) store.touch?.();
    return changed.changes === 1;
  }
  async function settlementStatements(input: TranscriptMapSettlementInput): Promise<{ statements: SqlStatement[]; summaryId: string | null }> {
    ...
    if (!details || json({ ...details, context: null }) !== json({ ...input.details, context: null })) throw new Error("transcript map work inputs changed");
    const { work: item, version, node: target, plan } = details;
    if ((item.mode === "review") !== (result.kind === "review")) throw new Error("transcript map result kind does not match work");
    const rows = await db.query<{ machine_id: string; policy: string }>(`SELECT machine_id,policy FROM transcript_map_versions WHERE id=?`, [version.id]);
    ...
    if (!await eligible(details, policy)) throw new Error("transcript map work no longer eligible");
    const check = readiness(details);
    const guard: TranscriptMapCondition = { sql: `(${input.guard.sql}) AND EXISTS (SELECT 1 FROM transcript_map_work WHERE id=? AND state='running' AND run_id=?) AND (${check.sql})`,
    ...
    if (result.kind === "review") {
      ...
      statements.push({ sql: `INSERT OR IGNORE INTO transcript_map_reviews(work_id,summary_id,verdict,reason,run_id,served_seq,created_at)
        SELECT ?,?,?,?,?,served_seq,? FROM transcript_map_work WHERE id=? AND ${guard.sql}`,
        params: [item.id, item.baseSummaryId, result.verdict, result.reason, input.runId, input.now, item.id, ...guard.params] });
    } else {
      const recipe = version.generateRecipe;
      summaryId = identity("tmsum", [item.id, input.runId]);
      const summary = TranscriptMapSummarySchema.parse({ id: summaryId, versionId: version.id, nodeId: target.id,
        text: result.text, runId: input.runId, recipeId: recipe.id, recipeVersion: recipe.version,
        recipeDigest: digest(recipe), profile: version.profile, children: item.children,
        supersedes: item.baseSummaryId, correctionDepth: item.correctionDepth, createdAt: input.now });
      const key = inputKey(plan, target, version, item.children);
      statements.push({ sql: `INSERT OR IGNORE INTO transcript_map_summaries(id,version_id,node_id,reuse_key,payload,text,created_at)
        SELECT ?,?,?,?,?,?,? WHERE ${guard.sql}`, params: [summaryId, version.id, target.id, key, json(summary), summary.text, input.now, ...guard.params] });
      statements.push({ sql: `INSERT INTO transcript_map_bindings(version_id,node_id,summary_id,input_key)
      ...
      if (item.baseSummaryId) statements.push({ sql: `UPDATE transcript_map_bindings SET summary_id=?
        ...
        params: [summaryId, item.baseSummaryId, key, ...input.guard.params, item.id, input.runId, summaryId] });
    }
    // Readiness changes with the binding above. Completion instead requires the artifact itself.
    statements.push({ sql: `UPDATE transcript_map_work SET state='complete' WHERE id=? AND state='running' AND run_id=? AND (${input.guard.sql})
  ...
  async function failureStatements(input: { workId: string; now: string; guard: TranscriptMapCondition; reason: string }): Promise<SqlStatement[]> {
    ...
    const ready = new Date(Date.parse(input.now) + 60_000 * 2 ** (details.work.attempt - 1)).toISOString();
    // Never persist reason: callers may accidentally pass a rejected provider body as an error.
    return [{ sql: `UPDATE transcript_map_work SET state=?,attempt=?,payload=json_set(payload,'$.attempt',?),
      ready_at=?,run_id=NULL,fence=NULL,claim_id=NULL,reason='attempt failed'
      WHERE id=? AND state='running' AND (${input.guard.sql})`,
      params: [exhausted ? "failed" : "queued", exhausted ? details.work.attempt : details.work.attempt + 1,
        exhausted ? details.work.attempt : details.work.attempt + 1, ready, input.workId, ...input.guard.params] }];
  }

  async function versionCoverage(plan: TranscriptMapPlan, versionId: string | null): Promise<TranscriptMapCoverage> {
    ...
    const rows = await db.query<{ summarized: number; tail: number; missing: number }>(`SELECT
      ...
      WHERE n.plan_id=?`, [versionId, plan.id]);
      ...
      (SELECT 1 FROM json_each(json_extract(s.payload,'$.children')) child
       ...
       (SELECT 1 FROM transcript_map_nodes parent JOIN transcript_map_bindings cb
        ...
        WHERE parent.id=b.node_id AND cb.summary_id=json_extract(child.value,'$.summaryId')
          AND NOT EXISTS (SELECT 1 FROM transcript_map_reviews r WHERE r.summary_id=cb.summary_id AND r.verdict IN ('correct','reject'))))`, [versionId]);
    out.stale = Number(stale[0]?.n ?? 0) > 0;
    const pending = await db.query<{ n: number }>(`SELECT count(*) n FROM transcript_map_bindings b
      WHERE b.version_id=? AND EXISTS (SELECT 1 FROM transcript_map_reviews r
        WHERE r.summary_id=b.summary_id AND r.verdict IN ('correct','reject'))`, [versionId]);
    out.stale ||= Number(pending[0]?.n ?? 0) > 0;
    if (versionId) {
      const current = await db.query<{ n: number }>(`SELECT count(*) n FROM transcript_map_heads WHERE version_id=?`, [versionId]);
      out.stale ||= Number(current[0]?.n ?? 0) === 0;
      const generation = await db.query<{ n: number }>(`SELECT count(*) n FROM transcript_map_versions v
        WHERE v.id=? AND v.generation<(SELECT coalesce(max(generation),0) FROM transcript_map_regenerations WHERE capture_id=?)`, [versionId, plan.source.id]);
      out.stale ||= Number(generation[0]?.n ?? 0) > 0;
    }
    out.partial ||= out.stale;
    return out;
  }
  async function coverage(scope: TranscriptMapScope, captureId?: string): Promise<TranscriptMapCoverage> {
    ...
    const rows = await db.query<{ capture_id: string; plan_id: string | null; version_id: string | null }>(
      ...
      [scope.machineId, ...auth.params, ...(captureId === undefined ? [] : [captureId]), MAX_SCAN + 1]);
    const out = emptyCoverage();
    const levels = new Set<number>();
    for (const row of rows.slice(0, MAX_SCAN)) {
      const plan = row.plan_id ? await one<TranscriptMapPlan>("transcript_map_plans", row.plan_id) : null;
      if (!plan) {
        out.partial = true;
        out.tailBytes = null;
        continue;
      ...
      if (out.tailBytes !== null) out.tailBytes = part.tailBytes === null ? null : out.tailBytes + part.tailBytes;
      for (const level of part.levels) levels.add(level);
    }
    out.levels = [...levels].sort((a, b) => a - b);
    if (rows.length > MAX_SCAN || (captureId === undefined && rows.length < scope.context.eligibleCaptures) ||
  ...
  async function status(scope: TranscriptMapScope): Promise<TranscriptMapStatus> {
    ...
    const rows = await db.query<{ n: number }>(`SELECT count(*) n FROM transcript_map_plans p
      LEFT JOIN transcript_map_heads h ON h.plan_id=p.id AND h.machine_id=?
    ...
    const known = await db.query<{ n: number }>(`SELECT count(*) n FROM transcript_map_contexts WHERE machine_id=? AND digest=?`, [scope.machineId, scope.context.digest]);
    const verifiedMappedCaptures = Number(rows[0]?.n ?? 0);
    return { eligibleCaptures: scope.context.eligibleCaptures, verifiedMappedCaptures,
      observedAt: scope.context.observedAt,
      partial: Number(known[0]?.n ?? 0) === 0 || verifiedMappedCaptures !== scope.context.eligibleCaptures };
  }

  async function node(scope: TranscriptMapScope, versionId: string, nodeId: string, knownCoverage?: TranscriptMapCoverage): Promise<TranscriptMapView | null> {
    const auth = authorization(scope, "p.capture_id");
    const visible = await db.query<{ plan_id: string }>(`SELECT v.plan_id FROM transcript_map_versions v
    ...
    const view = summary === null ? null : (() => {
      const { children: inputs, ...provenance } = summary;
      return { ...provenance, inputSummaryIds: inputs.flatMap((child) => child.summaryId ? [child.summaryId] : []) };
    })();
    return { inference: true, versionId, source: plan.source, node: item, summary: view,
  ...
  async function children(scope: TranscriptMapScope, versionId: string, nodeId: string): Promise<TranscriptMapView[]> {
  ...
  async function ancestors(scope: TranscriptMapScope, versionId: string, nodeId: string): Promise<TranscriptMapView[]> {
  ...
  async function search(scope: TranscriptMapScope, query: string, limit: number): Promise<TranscriptMapView[]> {
    ...
    const rows = await db.query<{ version_id: string; node_id: string }>(`SELECT b.version_id,b.node_id
      ...
      AND NOT EXISTS (SELECT 1 FROM transcript_map_reviews r WHERE r.summary_id=b.summary_id AND r.verdict IN ('correct','reject'))
      ORDER BY bm25(transcript_map_terms),b.version_id,b.node_id LIMIT ?`, [match, scope.machineId, ...auth.params, cap + 1]);
  ...
  async function noteServed(input: { readId: string; summaryIds: readonly string[]; now: string }): Promise<void> {
    if (!input.readId || input.readId.length > 512 || input.summaryIds.length > READ_LIMIT) throw new Error("invalid transcript map served event");
    await batch([...new Set(input.summaryIds)].map((id) => ({
  ...
  async function regenerate(input: { captureId: string; requestId: string; reason: string; now: string }): Promise<number> {
    if (!input.requestId || input.requestId.length > 512 || !input.reason.trim() || input.reason.length > 2000) throw new Error("invalid transcript map regeneration request");
    safeText(input.reason);
    await db.run(`INSERT OR IGNORE INTO transcript_map_regenerations(capture_id,request_id,generation,reason,created_at)
