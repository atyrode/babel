...
async function open(dir?: string, context: ReadingContext = CONTEXT): Promise<{ index: SessionIndex; dir: string }> {
  if (dir === undefined) {
...
function candidate(id: string, size = 10): IndexedSession {
  ...
  return { session: sessionRef("omp", id, `/unopened-session-index-fixture/${id}.jsonl`), seen: { size, modifiedAt: 1000 } };
}

function reading(seen: Observation): Reading {
  return { bytes: seen.size, records: 1, captureDigest: `sha256:${"a".repeat(64)}`,
    sourceDigest: `sha256:${"b".repeat(64)}`, report: null };
}
...
test("concurrent builders never duplicate a source read and readers retain the committed generation", async () => {
  ...
  try {
    const forbidden = async (sink: RecordSink) => { reads += 1; return content(next, "wrong")(sink); };
    ...
    expect(second.search("previous", [old], 10, MAX_MATERIAL_BYTES).selection).toEqual([old.session]);
  ...
  expect(await second.build(next, async (sink) => {
    reads += 1;
    return content(next, "wrong")(sink);
  })).toBe("reused");
  expect(reads).toBe(1);
  expect(second.holds(old)).toBe(false);
  expect(second.search("replacement", [next], 10, MAX_MATERIAL_BYTES).selection).toEqual([next.session]);
...
test("a failed replacement rolls back its tokens and releases the writer for another handle", async () => {
  ...
  await expect(failure).rejects.toMatchObject({ kind: "unavailable", message: "Session content index is unavailable." });
...
test("source identity, observation and every reading context field participate in coverage", async () => {
  ...
  let reads = 0;
  expect(await index.build({ ...original, seen: { size: 10, modifiedAt: 0 } }, async (sink) => {
    reads += 1;
    return content(original, "wrong")(sink);
  })).toBe("changed");
  expect(reads).toBe(0);
  for (const context of [{ ...CONTEXT, schema: 2 }, { ...CONTEXT, detectors: "different" }]) {
  ...
  await expect(sessionIndex(dir, { ...CONTEXT, mode: "off" })).rejects.toMatchObject({ kind: "unavailable" });
...
test("literal queries find escaped and UTF-8 content across arbitrary replay boundaries", async () => {
  ...
  await index.build(escaped, async (sink) => {
    const bytes = new TextEncoder().encode('{"text":"needle caf\\u00e9 café \\ud83d\\ude80\\n東京"}\n!literal\\backslash\n');
  ...
  expect(index.search('"***()"', [escaped, absent], 10, 100)).toEqual({ selection: [], matches: 0, overBound: 0 });
});
...
test("invalid or oversized normalized records refuse atomically, even if the callback swallows a sink error", async () => {
  ...
  await expect(index.build(next, async (sink) => {
    sink.write('{"text":"partial"}\n');
    try { sink.write('{"text":invalid}\n'); } catch { /* A faulty producer must not publish partial coverage. */ }
    return { reading: reading(next.seen), after: next.seen };
...
test("search counts only distinct eligible identities, preserves original refs and skips byte-overbound hits", async () => {
  ...
  expect(index.search("matching", [c, b, a], 1, 10)).toEqual({ selection: [b.session], matches: 3, overBound: 1 });
  expect(index.search("matching", [c, b], 1, 100)).toEqual({ selection: [b.session], matches: 2, overBound: 0 });
  expect(index.search("matching", [], 10, 100)).toEqual({ selection: [], matches: 0, overBound: 0 });
});
...
test("ranking uses the best matching passage, not recency or the number of matching passages", async () => {
  ...
  await index.build(weak, async (sink) => {
    for (let i = 0; i < 20; i += 1) sink.write(`${JSON.stringify({ text: `needle ${"padding ".repeat(100)}` })}\n`);
    return { reading: reading(weak.seen), after: weak.seen };
  });
  await index.build(strong, content(strong, "needle"));
  expect(index.search("needle", [weak, strong], 1, 100)).toEqual({ selection: [strong.session], matches: 2, overBound: 0 });
});
