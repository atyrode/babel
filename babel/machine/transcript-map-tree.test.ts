...
async function tree(text: string, options: Partial<TranscriptMapSegmentation> = {}) {
  ...
  return buildTranscriptMap({
    ...
    captureDigest: sha(text),
    sourceDigest: sha(text),
    segmentation: TranscriptMapSegmentationSchema.parse(options),
    async replay(sink) {
      // Force framing across UTF-8 and record boundaries.
      for (let offset = 0; offset < bytes.length; offset += 7)
        sink.write(bytes.subarray(offset, offset + 7));
      await sink.close();
    },
...
test("every leaf and parent digest names its exact contiguous canonical byte span", async () => {
  ...
  for (const leaf of leaves) {
    expect(leaf.span.byteOffset).toBe(offset);
    offset += leaf.span.byteLength;
  }
  for (const node of built.nodes) {
    expect(node.span.digest).toBe(
      sha(bytes.subarray(node.span.byteOffset, node.span.byteOffset + node.span.byteLength)),
    );
    expect(node.span.anchor.digest).toBe(
      sha(bytes.subarray(node.span.byteOffset, node.span.byteOffset + node.span.anchor.byteLength)),
    );
...
test("empty/direct captures and oversized records never create partial record summaries", async () => {
  ...
  const direct = await tree(record("a".repeat(600)) + record("b".repeat(600)), {
    leafBytes: 2048,
    directBytes: 2048,
  });
  expect(direct.header.direct).toBe(true);
  expect(direct.nodes).toHaveLength(1);
  const oversized = await tree(record("a".repeat(1400)) + record("short"), {
    leafBytes: 1024,
    directBytes: 256,
  });
...
test("depth overflow is one explicit contiguous gap and source growth preserves completed prefix spans", async () => {
  ...
  const capped = await tree(rows.join(""), {
    leafBytes: 1024,
    directBytes: 0,
    fanout: 2,
    maxDepth: 2,
  ...
  expect(after.nodes.slice(0, 3).map((node) => node.span)).toEqual(
    before.nodes.slice(0, 3).map((node) => node.span),
  );
