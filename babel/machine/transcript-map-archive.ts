...
export function transcriptMapArchive(options: {
  ...
  ): Promise<{ captures: MappingCapture[]; inventory: unknown }>;
  ...
  const load = async (
    ...
    if (
      existing &&
      (existing.reading.captureDigest !== reading.captureDigest ||
        existing.reading.sourceDigest !== reading.sourceDigest ||
        existing.reading.stream !== reading.stream)
    ) {
    ...
    if (
      existing &&
      JSON.stringify(existing.tree.header.segmentation) === JSON.stringify(segmentation)
    ) {
    ...
    try {
      ...
      db.exec(
        "CREATE TABLE records (line INTEGER PRIMARY KEY, offset INTEGER NOT NULL, bytes INTEGER NOT NULL, digest TEXT NOT NULL, time TEXT)",
      );
      ...
      const value: Kept = {
        reading,
        source: tree.header.source,
        db,
        directory: owned,
        tree,
        offsets: new Map(tree.nodes.map((node, offset) => [node.id, offset])),
        expires: options.now() + RECALL_REQUEST_TTL_MS,
      };
  ...
  const position = (value: Kept, line: number): SessionRecordPosition | null =>
    value.db
      .query<SessionRecordPosition, [number]>(
  ...
  const readSpan = async (
    ...
    if (
      !first ||
      !last ||
      JSON.stringify(first) !== JSON.stringify(span.anchor) ||
      first.byteOffset !== span.byteOffset ||
      last.byteOffset + last.byteLength !== span.byteOffset + span.byteLength
    )
      throw new Refused("locator-mismatch");
    if (span.byteLength > TRANSCRIPT_MAP_MAX_SPAN_BYTES) throw new Refused("fetch-bound");
    const bytes = new Uint8Array(
      await Bun.file(value.reading.stream)
        .slice(span.byteOffset, span.byteOffset + span.byteLength)
        .arrayBuffer(),
    );
    ...
    let offset = 0;
    const records = value.db.query<
      { line: number; offset: number; bytes: number; digest: string },
      [number, number]
    ...
    for (const record of records.iterate(span.firstRecord, span.lastRecord)) {
      if (
        record.line !== line++ ||
        record.offset !== span.byteOffset + offset ||
        record.bytes > bytes.length - offset
      )
        throw new Refused("capture-changed");
      const digest = `sha256:${createHash("sha256")
        .update(bytes.subarray(offset, offset + record.bytes))
        .digest("hex")}`;
      if (digest !== record.digest) throw new Refused("capture-changed");
      offset += record.bytes;
    }
    if (line !== span.lastRecord + 1 || offset !== bytes.length)
      throw new Refused("capture-changed");
  ...
  return {
    ...
    async execute(
      classId: string,
      input: TranscriptMapNativeRequest,
      privileged: boolean,
    ): Promise<TranscriptMapNativeResult> {
      const result: TranscriptMapNativeResult = {
      ...
      try {
        ...
        if (
          !["map-context", "map-inventory", "map-authorize", "map-span"].includes(request.kind) &&
        ...
        if (request.kind === "map-page" || request.kind === "map-release") {
          ...
          if (value.previous?.offset === request.offset) {
            if (Buffer.byteLength(value.previous.text) > request.maxBytes)
          ...
          const clipped = clipUtf8(
            value.text ?? "",
            Math.min(request.maxBytes, Math.floor((RECALL_MAX_PAYLOAD_BYTES - 4096) / 6)),
          );
        ...
        const context: TranscriptMapContext = {
          ...
          eligibleCaptures: inventory.captures.length,
          observedAt: new Date(options.now()).toISOString(),
        };
        result.context = context;
        const access = (entry: MappingCapture) => ({
        ...
        if (request.kind === "map-inventory") {
          ...
          if (request.cursor) {
            ...
            if (!match || match[1] !== digest([context.digest, classId]).slice(7))
              throw new Refused("stale-context");
            offset = Number(match[2]);
            if (!Number.isSafeInteger(offset) || offset > inventory.captures.length)
              throw new Refused("invalid-offset");
          }
          while (
            offset < inventory.captures.length &&
            ...
            if (Buffer.byteLength(JSON.stringify(result)) > RECALL_MAX_PAYLOAD_BYTES - 256) {
              result.entries.pop();
              break;
            }
            offset++;
          ...
          result.nextCursor =
            offset < inventory.captures.length
              ? `${digest([context.digest, classId]).slice(7)}:${offset}`
              : null;
          return result;
        }
        if (request.kind === "map-authorize") {
          for (const capture of request.captures) {
            try {
              result.accesses.push(access(selected(capture)));
            } catch {
              /* No authority for missing or reclassified captures. */
            }
          }
          return result;
        }
        ...
        const segmentation =
          request.kind === "map-plan" || request.kind === "map-node"
            ? request.segmentation
            : (kept.get(entry.capture.id)?.tree.header.segmentation ??
              TranscriptMapSegmentationSchema.parse({}));
        const value = await load(entry, segmentation, result.cost);
        if (request.kind === "map-node") {
          if (JSON.stringify(request.source) !== JSON.stringify(value.source))
            throw new Refused("locator-mismatch");
          const offset = value.offsets.get(request.nodeId);
          if (offset === undefined) throw new Refused("locator-mismatch");
          result.plan = {
            header: value.tree.header,
            nodes: [value.tree.nodes[offset]!],
            offset,
            nextOffset: null,
          };
          return result;
        ...
        if (request.kind === "map-plan") {
          ...
          while (
            Buffer.byteLength(JSON.stringify(result)) > RECALL_MAX_PAYLOAD_BYTES - 128 &&
            nodes.length
          )
            nodes.pop();
          if (!nodes.length && request.offset < value.tree.nodes.length)
        ...
        if (request.kind === "map-span") {
          ...
          const budget = Math.floor(
            (RECALL_MAX_PAYLOAD_BYTES - Buffer.byteLength(JSON.stringify(result)) - 128) / 6,
          );
          ...
          result.span.excerpt = {
            ...
            text: clipped.text,
            bytes: clipped.bytes,
            truncated: clipped.bytes < request.span.byteLength,
            firstRecord: clipped.bytes ? request.span.firstRecord : 0,
            lastRecord: clipped.bytes ? request.span.firstRecord + lines - 1 : 0,
          };
        } else {
          for (const [id, preview] of previews)
            if (preview.classId === classId && preview.text === null) previews.delete(id);
          ...
          result.preview = {
            previewId,
            bytes: request.span.byteLength,
            sourceDigest: value.source.sourceDigest,
            spanDigest: request.span.digest,
          };
        }
      } catch (error) {
