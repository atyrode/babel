...
function position(
  ...
  return {
    ...
    digest: `sha256:${new Bun.CryptoHasher("sha256").update(text).digest("hex")}`,
    time,
  };
}
...
test("UTF-8 prefixes never split Unicode or spend unused bytes on a later character", () => {
  ...
  for (const [limit, expected] of [
    ...
    expect(clipped).toEqual({
      text: expected,
      bytes: encoder.encode(expected).byteLength,
      truncated: expected !== text,
    });
  }
...
test("full-record preflight precedes prefix clipping for both evidence and metadata", async () => {
  ...
  const reader = recallRecordReader({
    harness: "omp",
    selection: { kind: "turns", first: 1, last: 1 },
    maxBytes: 45,
  });
...
test("anchor equality checks every coordinate, digest and archived timestamp", async () => {
  ...
  const record = stream([
    { type: "message", timestamp: time, message: { role: "user", content: "hello" } },
  ]);
  const exact = position(record, 1, 0, time);
  const wrong: SessionRecordPosition[] = [
    { ...exact, line: 2 },
    { ...exact, byteOffset: 1 },
    { ...exact, byteLength: exact.byteLength + 1 },
...
test("around counts physical normalized records and preserves opaque evidence", async () => {
  ...
  const reader = recallRecordReader({
    harness: "codex",
    anchor: position(at, 2, encoder.encode(before).byteLength),
...
test("Codex counts canonical user messages once, not duplicate delivery events", async () => {
  const records = [
    ...
    {
      type: "response_item",
      payload: { type: "message", role: "user", content: "canonical prompt" },
    },
    ...
    {
      type: "response_item",
      payload: { type: "message", role: "user", content: "second request" },
    },
    { type: "response_item", payload: { type: "message", role: "assistant", content: "done" } },
  ];
  const reader = recallRecordReader({
    harness: "codex",
    selection: { kind: "turns", first: 2, last: 2 },
  });
  reader.sink.write(stream(records));
  const result = await reader.finish();
...
test("Codex injected context does not shift requested user turns", async () => {
  ...
  const records = [
    ...
    {
      type: "response_item",
      payload: { type: "message", role: "assistant", content: "first answer" },
    },
    message("<environment_context>\n<cwd>/saved</cwd>\n</environment_context>"),
    message("second actual request"),
    {
      type: "response_item",
      payload: { type: "message", role: "assistant", content: "second answer" },
    },
  ];
  const reader = recallRecordReader({
    harness: "codex",
...
test("opaque archives cannot masquerade as turns or supply guessed metadata", async () => {
  ...
  for (const harness of ["omp", "codex", "claude"] as const) {
    const reader = recallRecordReader({
      harness,
      selection: { kind: "turns", first: 1, last: 10 },
    });
...
test("tiny excerpts stay bounded across oversized selected records while consuming the entire replay", async () => {
  const huge = stream([
    { type: "message", message: { role: "user", content: "x".repeat(2 << 20) } },
  ]);
  const reader = recallRecordReader({
    harness: "omp",
    selection: { kind: "turns", first: 1, last: 1000 },
    maxBytes: 7,
  });
  reader.sink.write(huge);
  const following = stream([
    { type: "message", message: { role: "assistant", content: "y".repeat(8192) } },
  ]);
  ...
  expect(result.bytes).toBe(
    encoder.encode(huge).byteLength + 128 * encoder.encode(following).byteLength,
  );
});
