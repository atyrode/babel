...
test("settlement between read and follow cannot turn a pending receipt into a failed run", async () => {
  const inference = {
    calls: 3, inputTokens: 100, outputTokens: 20, cachedInputTokens: 0, costMicros: 900,
  };
  const settled = {
    ...POSTED,
    state: "exited",
    result: {
      jobId: POSTED.jobId, requestDigest: "d".repeat(64),
      ownerId: "owner", ownerGeneration: 1, state: "exited", exitCode: 0, reason: null,
  ...
  const receipt = {
    sessionId: "session-7", sessionPath: "/outputs/session/session.jsonl",
    model: "anthropic/haiku", finalMessage: "Analysis complete.", usage: null, exitCode: 0,
  };
  let reads = 0;
  const engine = codeEngine(actions(({ action }) => {
    if (action === "readSession") {
      return ++reads === 1
        ? { job: { ...POSTED, state: "started" }, session: null }
        : { job: settled, session: receipt };
    }
    if (action === "followSession") {
      return { job: settled, inferenceUsage: { ...inference, lastModel: "anthropic/haiku" },
        progress: { stage: "at the model", at: 1_100 }, inferenceCalls: [],
        seq: 4, firstSeq: 1, unavailable: null };
    }
...
test("missing activity cannot erase a confirmed live session", async () => {
  const engine = codeEngine(actions(({ action }) => {
    if (action === "readSession") return { job: { ...POSTED, state: "started" }, session: null };
