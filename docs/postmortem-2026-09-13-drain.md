...
Already absent by design (with the line): whole-corpus digest per review (`machine/prepare.ts:1-15`, where a preparation is a fixed selection whose identity is derived from its content), the two-file lock storm (ADR 0034, `plugins/README.md`), policy-only-via-nonce (`coordinator.ts:154-169`), `--preparation` (`doors/launch.ts:237-278`), exit-code ambiguity (`results.ts:38-56`), no visibility before settle (manifold#543 `inference_call` on the follow stream), lease shorter than batch (`coordinator.ts:131-144, 229-247`).
