...
| F8 | **Evidence-role reviews were paid for and refused at submit**: the review contract requires an environment on criterion results (`v0.4.0:internal/explore/review.go:571`), the store refused any environment without an outcome (`v0.4.0:internal/evaluation/model.go:953`) and counted results-only assessments as empty. Three copies of the rule, no shared validator. Fixed 8f7e2cf for the two copies; B16 shows the third. | B16 | **verify** in `machine/results.ts` (D11 lists refusal codes `schema/authority/support/empty/…`): one validator shared by producer and store, and a refused submission still settles the claim and counts as spend (#263). |
...
Already absent by design (with the line): whole-corpus digest per review (`machine/prepare.ts:1-15`, where a preparation is a fixed selection whose identity is derived from its content), the two-file lock storm (ADR 0034, `docs/building.md`), policy-only-via-nonce (`coordinator.ts:154-169`), `--preparation` (`doors/launch.ts:237-278`), exit-code ambiguity (`results.ts:38-56`), no visibility before settle (manifold#543 `inference_call` on the follow stream), lease shorter than batch (`coordinator.ts:131-144, 229-247`).
...
- Tokens **are** recorded by the Go worker: `worker.Receipt.Usage{InputTokens, OutputTokens,
  ...
  (`v0.4.0:internal/worker/domain.go:329-334`, `rpc.go:383-388`, `receipt.go:96`), stored in the run
