...
On managed machines, `atyrode/dotfiles` owns secret custody and activation: shared `babel-custody` clan vars hold the existing repository password, provider environment inputs, and complete append-only payload key ring; per-machine `babel-archive` vars derive storage configuration using registry identity. sops-nix places the secret files and Home Manager links Babel's fixed config paths; no vault or provider session runs on the fleet machine. The operator generates missing vars on an authorized operator device, commits the encrypted update, then runs `atyrode apply` on the target. See the [current custody and recovery procedure](docs/runbook.md#3-restic-repository-password-custody), pinned to [dotfiles `f2a4749`, `modules/shared/babel-archive.nix:1–31`](https://github.com/atyrode/dotfiles/blob/f2a4749eab77ac859b85354142e42c78ac6d8c80/modules/shared/babel-archive.nix#L1-L31). Missing placement is repaired by applying existing custody, never replacing the ring.
