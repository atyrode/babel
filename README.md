...
The audited product and delivery specification is in [SPEC.md](SPEC.md). Babel will be implemented as a public Go application using Bubble Tea and [`atyrode/cli-kit`](https://github.com/atyrode/cli-kit), with a TypeScript/React web application compiled into the Go binary.

Running `babel` opens the operational terminal interface; `babel web` starts an on-demand loopback-only web interface. Development begins locally, but the first deployed v1 is multi-machine: every authorized Babel instance connects to one logical shared backend composed of PostgreSQL for catalog/coordination/structured state and S3-compatible object storage for immutable encrypted bytes. The operator deployment uses Clever Cloud PostgreSQL plus the existing Cellar bucket; provider-neutral local directory/SQLite modes remain available for fixtures and recovery. Phase A freezes the archive contract before remote writes, proves all three harnesses, and exposes the shared metadata catalog through terminal and thin web surfaces. Open-ended Code→OMP exploration follows on top of that shared foundation.
...
- **One logical backend, two storage primitives.** Shared PostgreSQL holds global catalog, coordination, relationships, and structured state; S3-compatible storage holds immutable raw chats, attachments, evidence, snapshots, and large outputs. Every authorized machine uses the same backend, while local SQLite/files remain rebuildable cache, search index, scratch, and pending-sync state.
- **Archive recovery does not depend on PostgreSQL.** Immutable S3 commit records remain sufficient to rebuild the Phase A shared catalog; PostgreSQL becomes the visibility boundary for Phase B analysis/Reality/review commits.
