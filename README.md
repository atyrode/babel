...
- **The hub is the one place.** Nothing is sealed into a second store and nothing is synced to another host, and "published" is a word the schema does not need. One place is also one copy: its backup is an image in the session transcripts' restic repository, never a second store, and until it lands (#454) losing the hub's volume loses every record.
