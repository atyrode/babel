...
func (s *Store) BeginAttempt(ctx context.Context, id string) (func(), error) {
 ...
 return func() { close(stop); <-done; _, _ = s.db.ExecContext(context.Background(), `DELETE FROM run_lease WHERE run_id = ? AND host = ? AND pid = ?`, id, host, os.Getpid()) }, nil
}
...
func (s *Store) Interrupted(ctx context.Context) ([]Receipt, error) {
 ...
 for offset := 0; ; {
  ...
  if err != nil { return nil, err }
  for _, r := range page { if r.Body.Checkpoint != nil && r.Body.Checkpoint.State == Interrupted { out = append(out, r) } }
  offset += len(page)
  if offset >= total || len(page)==0 { return out, nil }
 }
}
...
func (s *Store) Reconcile(ctx context.Context, before time.Time) ([]Receipt, error) {
 ...
 if err != nil { return nil, err }
 type candidate struct { id string; pid int; beat string }
 var candidates []candidate
 for rows.Next() { var c candidate; if err := rows.Scan(&c.id, &c.pid, &c.beat); err != nil { rows.Close(); return nil, err }; candidates = append(candidates, c) }
 ...
 for _, c := range candidates {
  ...
  if !errors.Is(err, os.ErrProcessDone) && !errors.Is(err, syscall.ESRCH) { continue }
  prior, err := s.Latest(ctx, c.id)
  if err != nil {
   if !errors.Is(err, ErrNotFound) { return out, err }
   ...
   if _, err := s.db.ExecContext(ctx, `DELETE FROM run_lease WHERE run_id=? AND heartbeat=? AND pid=?`, c.id, c.beat, c.pid); err != nil { return out, err }
   continue
  }
  ...
  if err = s.DeclareClosure(ctx, c.id); err != nil { return out, err }
  _, err = s.db.ExecContext(ctx, `DELETE FROM run_lease WHERE run_id = ? AND heartbeat = ? AND pid = ?`, c.id, c.beat, c.pid)
  if err != nil { return out, err }
  if next.Body.Checkpoint.State == Interrupted { out = append(out, next) }
 }
 return out, nil
}
...
func (s *Store) CloseInterrupted(ctx context.Context, id string) (Receipt, error) {
 ...
 if err != nil { return Receipt{}, err }; defer release()
 prior, err := s.Latest(ctx, id)
 if err != nil { return Receipt{}, err }
 if prior.Body.Checkpoint == nil || prior.Body.Checkpoint.State != Interrupted { return Receipt{}, fmt.Errorf("run: only an interrupted run can be closed") }
...
func (s *Store) KnownRecords(ctx context.Context, id string) ([]string, string, error) {
 var exists int
 if err := s.db.QueryRowContext(ctx, `SELECT count(*) FROM sqlite_master WHERE type='table' AND name='explore_commit'`).Scan(&exists); err != nil { return nil, "", err }
 if exists==0 { return nil, "", nil }
 ...
 for rows.Next() {
  var record string
  if err := rows.Scan(&record, &stage); err != nil { return nil, "", err }
  if !seen[record] { seen[record]=true; records=append(records,record) }
 }
