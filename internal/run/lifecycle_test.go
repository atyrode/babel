...
func lifecycleReceipt(t *testing.T, s *Store, id string, state Lifecycle) Receipt {
 ...
 if err := s.PutReceipt(t.Context(), r); err != nil { t.Fatal(err) }
 return r
}
...
func TestInterruptedCloseListsAndExcludesConcurrentResume(t *testing.T) {
 ...
 release, err := s.BeginAttempt(t.Context(),"partial"); if err != nil { t.Fatal(err) }
 if _, err := s.CloseInterrupted(t.Context(),"partial"); err==nil { t.Fatal("close raced an owned attempt") }
 release()
 listed, err := s.Interrupted(t.Context()); if err != nil || len(listed)!=1 { t.Fatalf("list = %v %v",listed,err) }
 closed, err := s.CloseInterrupted(context.Background(),"partial"); if err != nil { t.Fatal(err) }
 if closed.Header.Supersedes!=prior.Header.ID || closed.Body.Checkpoint.State!=Closed { t.Fatal("close did not append a terminal amendment") }
 listed, err = s.Interrupted(t.Context()); if err != nil || len(listed)!=0 { t.Fatalf("closed run still interrupted: %v %v",listed,err) }
 if _, err := s.CloseInterrupted(t.Context(),"partial"); err==nil { t.Fatal("closed a terminal run twice") }
}
...
func TestRunningReceiptCannotPublishAsFinished(t *testing.T) {
 hook := sync.NewStager()
 s, err := Open(t.TempDir(),WithSync(hook)); if err != nil { t.Fatal(err) }; defer s.Close()
 prior := lifecycleReceipt(t,s,"still-running",Running)
 declared, _, err := s.DeclareFinished(t.Context(),hook)
 if err != nil || len(declared)!=0 { t.Fatalf("running receipt declared a closure: %v %v",declared,err) }
 interrupted, err := s.Transition(t.Context(),prior,Interrupted,"operator stop"); if err != nil { t.Fatal(err) }
 declared, _, err = s.DeclareFinished(t.Context(),hook)
 if err != nil || len(declared)!=1 || declared[0]!="still-running" { t.Fatalf("partial receipt not publishable: %v %v",declared,err) }
 closed, err := s.CloseInterrupted(t.Context(),"still-running"); if err != nil { t.Fatal(err) }
 var continues string
 if err := s.db.QueryRow(`SELECT continues_run_id FROM sync_run WHERE run_id=?`,closed.Header.ID).Scan(&continues); err != nil { t.Fatal(err) }
 if continues!="still-running" || closed.Header.Supersedes!=interrupted.Header.ID { t.Fatal("intentional close lost the immutable continuation lineage") }
 if _, err := s.Transition(t.Context(),closed,Resumed,"attempt to reopen"); err==nil { t.Fatal("closed run was resumed") }
}
