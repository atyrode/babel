...
func TestStopFileHonorsTheCycleBoundary(t *testing.T) {
	...
	if err := loop.Run(t.Context(), conductor.RunOptions{StopFile: path}); err != nil {
		t.Fatal(err)
	}
	if !finished || len(runner.runs) != 1 {
		t.Fatal("stop file did not finish exactly the active cycle")
	}
	if err := loop.Run(t.Context(), conductor.RunOptions{StopFile: path}); err != nil {
		t.Fatal(err)
	}
	if len(runner.runs) != 1 {
		t.Fatal("an existing stop file allowed another launch")
	}
}
