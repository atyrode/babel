...
func TestAttemptClaimedAfterAdmissionLeavesCyclePending(t *testing.T) {
 ...
 if !errors.Is(err,runstore.ErrAttemptOwned) { t.Fatalf("admission race = %v",err) }
 last, ok := journal.Last()
 if !ok || last.Outcome!=conductor.OutcomeRunning || last.RunID!=first.RunID { t.Fatal("lease contention permanently failed the claimed cycle") }
 second, err := loop.Once(t.Context())
 if !errors.Is(err,runstore.ErrAttemptOwned) { t.Fatalf("pending retry = %v",err) }
 if second.RunID!=first.RunID || second.Seq!=first.Seq || rung.draws!=1 { t.Fatal("pending recovery abandoned the original assignment") }
}
