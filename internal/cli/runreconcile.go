...
func (a *app) reconcileHistorical(ctx context.Context, runs *runstore.Store, age time.Duration) ([]runstore.Receipt, error) {
	...
	if age < presence.LostAfter {
		age = presence.LostAfter
	}
	excluded := map[string]bool{}
	newest := map[string]presence.Row{}
	for _, row := range rows {
		if !row.Local || row.State != presence.StateRunning || row.HeartbeatAge < age || row.ReceiptRecordID != "" {
			excluded[row.RunID] = true
		}
		if row.Kind != presence.KindExplore {
			continue
		}
		prior, exists := newest[row.RunID]
		if !exists || row.HeartbeatAt.After(prior.HeartbeatAt) {
			newest[row.RunID] = row
		}
	}
	out := []runstore.Receipt{}
	for id, row := range newest {
		if excluded[id] || row.PreparationID == "" {
			continue
		}
		receipt, err := runs.RecoverHistorical(ctx, id, runstore.PreparationID(row.PreparationID), row.Authority, row.Recipe, row.StartedAt)
		if err != nil {
			return out, err
		}
		if receipt != nil {
