...
func recordedExplorePlan(receipt runstore.Receipt, wcfg worker.Config) (explorePlan, error) {
 ...
 if cp==nil || cp.State!=runstore.Interrupted { return explorePlan{}, fmt.Errorf("run is not interrupted; reconcile an orphan before resuming") }
 if cp.Launch==nil { return explorePlan{}, fmt.Errorf("historical run has no trusted launch checkpoint; use babel explore --run-id with explicitly supplied preparation, recipes and profile") }
 launch := cp.Launch
 if len(launch.Recipes)==0 || launch.Profile.ID=="" || launch.Profile.Revision<1 { return explorePlan{}, fmt.Errorf("recorded launch inputs are incomplete") }
 set, err := recipeSet(launch.Recipes)
 if err != nil { return explorePlan{}, err }
 for _, recipe := range set.All() {
  ...
  if !found { return explorePlan{}, fmt.Errorf("recorded cookbook version is unavailable; refusing to substitute resume inputs") }
 }
 return explorePlan{
