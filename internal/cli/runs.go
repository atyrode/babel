...
func (a *app) runsCmd(ctx context.Context, args []string) error {
	...
	if len(args) == 0 {
		return c.usagef("runs requires a command")
	}
	verb := args[0]
	args = args[1:]
	var id string
	if verb == "resume" || verb == "close" {
		if len(args) == 0 {
			return c.usagef("%s requires a run ID", verb)
		}
		id, args = args[0], args[1:]
	} else if verb != "interrupted" && verb != "reconcile" {
		return c.usagef("unknown runs command")
	}
	...
	if *stale < time.Minute {
		return c.usagef("stale-after must be at least one minute")
	}
	...
	switch verb {
	case "interrupted":
		receipts, err = state.runs.Interrupted(ctx)
	case "reconcile":
	...
	case "resume":
		...
		if cp == nil || cp.State != runstore.Interrupted {
			return fmt.Errorf("run is not interrupted; reconcile an orphan before resuming")
		}
		if cp.Launch == nil {
			return fmt.Errorf("historical run has no trusted launch checkpoint; use babel explore --run-id with explicitly supplied preparation, recipes and profile")
		}
		launch := cp.Launch
		if len(launch.Recipes) == 0 || launch.Profile.ID == "" {
			return fmt.Errorf("recorded launch inputs are incomplete")
		}
		...
		for _, recipe := range set.All() {
			...
			if !found {
				return fmt.Errorf("recorded cookbook version is unavailable; refusing to substitute resume inputs")
			}
		}
		...
		if err := refuseDials(c, wcfg.Args, true); err != nil {
			return err
		}
		ctx, stop := signal.NotifyContext(ctx, os.Interrupt, syscall.SIGTERM)
		defer stop()
		announcer, release := a.openPresence(ctx)
		defer release()
	...
	if verb != "interrupted" {
		for _, r := range receipts {
			a.publishLifecycle(context.WithoutCancel(ctx), r.Header.RunID)
		}
	}
	if *asJSON {
		return a.emitJSON(receipts)
	}
	for _, r := range receipts {
