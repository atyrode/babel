...
func (completionUnusedRung) Depth(context.Context) (conductor.Depth, error) { return conductor.Depth{}, nil }
...
func TestConductorRecoversReceiptBeforeCycleFinalizationWithoutInference(t *testing.T) {
	for _, tc := range []struct { name, failure string; cancelled bool; outcome conductor.Outcome }{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err != nil { t.Fatal(err) }
			if err := store.PutReceipt(t.Context(),receipt); err != nil { t.Fatal(err) }
			...
			if _, err := loop.Once(t.Context()); !errors.Is(err,blocked) { t.Fatalf("next cycle should enforce its own budget: %v",err) }
