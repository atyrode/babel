...
| Suites              | `bun test`                                                                                                          | Requires `restic` on `PATH` for disposable synthetic archive fixtures. Tests the manifests against `babel/contract.ts` and `babel/store/schema.ts`, doors against a real temporary database, panels in a document, and `pack` itself.                                                                        |
