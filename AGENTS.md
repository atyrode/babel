...
| Format and lint     | `bun run lint`; `bun run format:check`; `bun run format` to fix                                                     | ESLint and Prettier at the Manifold sibling's own versions and settings, so three repositories in lockstep keep one convention rather than three. Two `react-hooks` rules warn rather than error; `eslint.config.js` says which four defects they found and what owns fixing them.                           |
